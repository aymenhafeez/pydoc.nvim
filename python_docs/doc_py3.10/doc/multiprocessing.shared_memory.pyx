...
class multiprocessing.shared_memory.SharedMemory(name=None, create=False, size=0)
   ...
   _name_ is the unique name for the requested shared memory,
   ...
   _create_ controls whether a new shared memory block is created
   ("True") or an existing shared memory block is attached ("False").

   _size_ specifies the requested number of bytes when creating a new
...
class multiprocessing.managers.SharedMemoryManager([address[, authkey]])
   ...
   of the inherited _address_ and _authkey_ optional input arguments
