Python 3.10.19
*mailcap.pyx*                                 Last change: 2025 Dec 20
...
Deprecated since version 3.11: The "mailcap" module is deprecated (see
...
or web document with the MIME type _video/mpeg_, "%s" will be replaced
...
mailcap.findmatch(caps, MIMEtype, key='view', filename='/dev/null', plist=[])
   ...
   Changed in version 3.10.8: To prevent security issues with shell
   ...
   line.If a disallowed character appears in _filename_, "findmatch"
   will always return "(None, None)" as if no entry was found. If such
   a character appears elsewhere (a value in _plist_ or in
   _MIMEtype_), "findmatch" will ignore all mailcap entries which use
