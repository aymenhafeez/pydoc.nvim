Python 3.10.19
*asyncore.pyx*                                Last change: 2025 Dec 20
...
Deprecated since version 3.6: "asyncore" will be removed in Python
...
asyncore.loop([timeout[, use_poll[, map[, count]]]])
   ...
   The _use_poll_ parameter, if true, indicates that "poll()" should
...
<
   ...
   handle_accepted(sock, addr)
      ...
      a "connect()" call for the local endpoint.  _sock_ is a _new_
      socket object usable to send and receive data on the connection,
      and _addr_ is the address bound to the socket on the other end
      of the connection.
   ...
   create_socket(family=socket.AF_INET, type=socket.SOCK_STREAM)
      ...
      Changed in version 3.3: _family_ and _type_ arguments can be
      omitted.

   connect(address)

      As with the normal socket object, _address_ is a tuple with the
   ...
   recv(buffer_size)
      ...
      Read at most _buffer_size_ bytes from the socket’s remote end-
   ...
   bind(address)
      ...
      refer to the "socket" documentation for more information.)  To
      mark the socket as re-usable (setting the "SO_REUSEADDR"
   ...
   accept()
      ...
      socket object usable to send and receive data on the connection,
      and _address_ is the address bound to the socket on the other
...
class asyncore.file_dispatcher
   ...
   A file_dispatcher takes a file descriptor or _file object_ along
