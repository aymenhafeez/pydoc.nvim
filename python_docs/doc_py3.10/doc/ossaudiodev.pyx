Python 3.10.19
*ossaudiodev.pyx*                             Last change: 2025 Dec 20

"ossaudiodev" — Access to OSS-compatible audio devices
******************************************************

Deprecated since version 3.11: The "ossaudiodev" module is deprecated
...
oss_audio_device.write(data)
   ...
   Write a _bytes-like object_ _data_ to the audio device and return
...
oss_audio_device.writeall(data)
   ...
   Write a _bytes-like object_ _data_ to the audio device: waits until
   the audio device is able to accept data, writes as much data as it
   will accept, and repeats until _data_ has been completely written.
...
oss_audio_device.setfmt(format)
   ...
   Try to set the current audio format to _format_—see "getfmts()" for
   a list.  Returns the audio format that the device was set to, which
   may not be the requested format.  May also be used to return the
   current audio format—do this by passing an “audio format” of
   "AFMT_QUERY".
...
oss_audio_device.setparameters(format, nchannels, samplerate[, strict=False])
   ...
   _nchannels_, and _samplerate_ should be as specified in the
   "setfmt()", "channels()", and "speed()"  methods.  If _strict_ is
   true, "setparameters()" checks to see if each parameter was
   actually set to the requested value, and raises "OSSAudioError" if
   not.  Returns a tuple (_format_, _nchannels_, _samplerate_)
