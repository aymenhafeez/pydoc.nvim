...
class graphlib.TopologicalSorter(graph=None)
   ...
   Provides functionality to topologically sort a graph of _hashable_
   nodes.
   ...
   If the optional _graph_ argument is provided it must be a
...
<
   add(node, *predecessors)
      ...
      _node_ and all elements in _predecessors_ must be _hashable_.

      If called multiple times with the same node argument, the set of
      dependencies will be the union of all dependencies passed in.

      It is possible to add a node with no dependencies
      (_predecessors_ is not provided) or to provide a dependency
      twice. If a node that has not been provided before is included
      among _predecessors_ it will be automatically added to the graph
...
<
      ...
      as processed, unblocking any successor of each node in _nodes_
      for being returned in the future by a call to
      "TopologicalSorter.get_ready()".

      Raises "ValueError" if any node in _nodes_ has already been
