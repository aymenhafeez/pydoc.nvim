...
list.index(x[, start[, end]])
   ...
   is equal to _x_. Raises a "ValueError" if there is no such item.

   The optional arguments _start_ and _end_ are interpreted as in the
   ...
   to the beginning of the full sequence rather than the _start_
   argument.
...
is also another standard sequence data type: the _tuple_.
...
_immutable_, and usually contain a heterogeneous sequence of elements
...
are _mutable_, and their elements are usually homogeneous and are
...
indexed by _keys_, which can be any immutable type; strings and
...
same sequence type. The comparison uses _lexicographical_ ordering:
