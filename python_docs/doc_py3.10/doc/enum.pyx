...
Note:
  ...
  * The attributes "Color.RED", "Color.GREEN", etc., are _enumeration
    members_ (or _enum members_) and are functionally constants.
...
>
   ...
   <Color.RED: 1>
<
The _type_ of an enumeration member is the enumeration it belongs to:
>
   >>> type(Color.RED)
   <enum 'Color'>
...
3. When another data type is mixed in, the "value" attribute is _not
   the same_ as the enum member itself, although it is equivalent and
   will compare equal.

4. %-style formatting:  _%s_ and _%r_ call the "Enum" class’s
   "__str__()" and "__repr__()" respectively; other codes (such as
   _%i_ or _%h_ for IntEnum) treat the enum member as its mixed-in
   type.
...
class, such as _list(Color)_ or _some_enum_var in Color_.  "EnumMeta"
...
underscore). Using these names in 3.10 will issue a
"DeprecationWarning".
...
Note:
  ...
  canonical flag members (aka single-value flags).  So "Color(7)" will
  produce something like "<Color.BLUE|GREEN|RED: 7>".
