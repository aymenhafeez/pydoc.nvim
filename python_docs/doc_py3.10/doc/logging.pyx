...
class logging.LogRecord(name, level, pathname, lineno, msg, args, exc_info, func=None, sinfo=None)
   ...
   Parameters:
      ...
      * **msg** (_typing.Any_) – The event description message, which
        can be a %-format string with placeholders for variable data,
        or an arbitrary object (see Using arbitrary objects as
        messages).

      * **args** (_tuple | dict__[__str__, __typing.Any__]_) –
        Variable data to merge into the _msg_ argument to obtain the
        event description.

      * **exc_info** (_tuple__[__type__[__BaseException__]__,
        __BaseException__, __types.TracebackType__] __| None_) – An
        exception tuple with the current exception information, as
        returned by "sys.exc_info()", or "None" if no exception
        information is available.

      * **func** (_str | None_) – The name of the function or method
        from which the logging call was invoked.

      * **sinfo** (_str | None_) – A text string representing stack
        information from the base of the stack in the current thread,
...
Changed in version 3.6: Attribute "manager" and method "_log()" were
added, which delegate to the underlying logger and allow adapters to
be nested.
...
logging.debug(msg, *args, **kwargs)
   ...
   There are three keyword arguments in _kwargs_ which are inspected:
   ...
   specifying _exc_info_: The former is stack frames from the bottom
   ...
   You can specify _stack_info_ independently of _exc_info_, e.g. to
...
<
   ...
   The third optional keyword argument is _extra_ which can be used to
...
<
   ...
   need to pass the _extra_ dictionary with these keys.
   ...
   This function (as well as "info()", "warning()", "error()" and
   "critical()") will call "basicConfig()" if the root logger doesn’t
