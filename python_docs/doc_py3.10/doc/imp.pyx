...
imp.get_suffixes()
   ...
   "'r'" for text files or "'rb'" for binary files), and _type_ is the
...
imp.find_module(name[, path])
   ...
   _file_ is an open _file object_ positioned at the beginning,
   ...
   If the module is built-in or frozen then _file_ and _pathname_ are
   both "None" and the _description_ tuple contains empty strings for
   ...
   package path and the last item in the _description_ tuple is
   "PKG_DIRECTORY".

   This function does not handle hierarchical module names (names
   containing dots).  In order to find _P.M_, that is, submodule _M_
   of package _P_, use "find_module()" and "load_module()" to find and
   load package _P_, and then use "find_module()" with the _path_
   argument set to "P.__path__". When _P_ itself has a dotted name,
...
imp.load_module(name, file, pathname, description)
   ...
   already imported, it will reload the module!  The _name_ argument
   indicates the full module name (including the package name, if this
   is a submodule of a package).  The _file_ argument is an open file,
   and _pathname_ is the corresponding file name; these can be "None"
   and "''", respectively, when the module is a package or not being
   loaded from a file.  The _description_ argument is a tuple, as
...
<
   ...
   qualified names (_module_.*name*) instead.
...
imp.cache_from_source(path, debug_override=None)
   ...
   with the source _path_.  For example, if _path_ is
   ...
   "False" for _debug_override_ you can override the system’s value
   for "__debug__", leading to optimized bytecode.

   _path_ need not exist.
   ...
   Changed in version 3.5: The _debug_override_ parameter no longer
   creates a ".pyo" file.
