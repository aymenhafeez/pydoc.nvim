...
_should_ make a major speedup possible—we could rewrite its code
...
* The first line of the comment ("/*[clinic input]") is the _start
  line_.

* The last line of the initial comment ("[clinic start generated
  code]*/") is the _end line_.

* The last line ("/*[clinic end generated code: checksum=...]*/") is
  the _checksum line_.

* In between the start line and the end line is the _input_.

* In between the end line and the checksum line is the _output_.

* All the text collectively, from the start line to the checksum line
  inclusively, is the _block_.  (A block that hasn’t been successfully
...
<
  ...
  choose a different function.  Argument Clinic _does_ support all of
...
<
  ...
  the "PyArg_Parse()" format argument and specify _that_ as its
...
* Double-check that the argument-parsing code Argument Clinic
  ...
  conversion function), ensure that the second argument is _exactly_
...
<
  ...
  If any of these items differ in _any way_, adjust your Argument
  Clinic function specification and rerun "Tools/clinic/clinic.py"
  until they _are_ the same.
...
* Remember the macro with the "PyMethodDef" structure for this
  ...
  macro, _don’t_ add a comma to the end.
...
_format units_, with specific behavior.  For example, currently you
...
"_PyType_GetModuleByDef" to look up the module and then
...
_must_ be keyword-only.  Any arguments passed in to the converter in
