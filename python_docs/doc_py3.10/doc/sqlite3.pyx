...
sqlite3.connect(database, timeout=5.0, detect_types=0, isolation_level='DEFERRED', check_same_thread=True, factory=sqlite3.Connection, cached_statements=100, uri=False)
   ...
   Parameters:
      ...
      * **isolation_level** (_str | None_) – The "isolation_level" of
   ...
   New in version 3.4: The _uri_ parameter.
...
sqlite3.register_adapter(type, adapter, /)
   ...
   Register an _adapter_ callable to adapt the Python type _type_ into
   an SQLite type. The adapter is called with a Python object of type
   _type_ as its sole argument, and must return a value of a type that
   SQLite natively understands.

sqlite3.register_converter(typename, converter, /)

   Register the _converter_ callable to convert SQLite objects of type
   _typename_ into a Python object of a specific type. The converter
   is invoked for all SQLite values of type _typename_; it is passed a
   "bytes" object and should return an object of the desired Python
   type. Consult the parameter _detect_types_ of "connect()" for
...
sqlite3.SQLITE_IGNORE
   ...
   Flags that should be returned by the _authorizer_callback_ callable
...
class sqlite3.Connection
   ...
   create_function(name, narg, func, *, deterministic=False)
      ...
      Parameters:
         ...
         * **func** (_callback_ | None) – A callable that is called
      ...
      New in version 3.8: The _deterministic_ parameter.
...
<
   ...
   set_authorizer(authorizer_callback)
      ...
      Register callable _authorizer_callback_ to be invoked for each
   ...
   set_progress_handler(progress_handler, n)
      ...
      Register callable _progress_handler_ to be invoked for every _n_
   ...
   set_trace_callback(trace_callback)
      ...
      Register callable _trace_callback_ to be invoked for each SQL
...
<
   backup(target, *, pages=- 1, progress=None, name='main', sleep=0.250)
      ...
      Parameters:
         ...
         * **progress** (_callback_ | None) – If set to a callable, it
           is invoked with three integer arguments for every backup
...
<
   ...
   text_factory
      ...
      "str". If you want to return "bytes" instead, set _text_factory_
      to "bytes".
...
class sqlite3.Cursor
   ...
   execute(sql, parameters=(), /)
      ...
      Execute SQL a single SQL statement, optionally binding Python
      ...
      Raises:
         **Warning** – If _sql_ contains more than one SQL statement.
   ...
   executemany(sql, parameters, /)
      ...
      parameterized SQL statement _sql_.

      Uses the same implicit transaction handling as "execute()".

      Parameters:
         * **sql** (_str_) – A single SQL DML (Data Manipulation
           Language) statement.
      ...
      Raises:
         * **ProgrammingError** – If _sql_ is not a DML statment.

         * **Warning** – If _sql_ contains more than one SQL
           statement.
