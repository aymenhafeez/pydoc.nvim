...
class wsgiref.util.FileWrapper(filelike, blksize=8192)
   ...
   A wrapper to convert a file-like object to an _iterator_.  The
   resulting objects support both "__getitem__()" and "__iter__()"
   iteration styles, for compatibility with Python 2.1 and Jython. As
   the object is iterated over, the optional _blksize_ parameter will
   be repeatedly passed to the _filelike_ object’s "read()" method to
