...
second edition of _Python Cookbook_, published by O’Reilly.
...
timeit.timeit(stmt='pass', setup='pass', timer=<default timer>, number=1000000, globals=None)
   ...
   Create a "Timer" instance with the given statement, _setup_ code
   and _timer_ function and run its "timeit()" method with _number_
   executions. The optional _globals_ argument specifies a namespace
...
timeit.repeat(stmt='pass', setup='pass', timer=<default timer>, repeat=5, number=1000000, globals=None)
   ...
   Create a "Timer" instance with the given statement, _setup_ code
   and _timer_ function and run its "repeat()" method with the given
   _repeat_ count and _number_ executions.  The optional _globals_
   argument specifies a namespace in which to execute the code.

   Changed in version 3.5: The optional _globals_ parameter was added.

   Changed in version 3.7: Default value of _repeat_ changed from 3 to
   5.
...
class timeit.Timer(stmt='pass', setup='pass', timer=<timer function>, globals=None)
   ...
   controlled by passing a namespace to _globals_.

   To measure the execution time of the first statement, use the
   "timeit()" method.  The "repeat()" and "autorange()" methods are
   convenience methods to call "timeit()" multiple times.

   The execution time of _setup_ is excluded from the overall timed
   execution run.

   The _stmt_ and _setup_ parameters can also take objects that are
   ...
   timeit(number=1000000)
      ...
      Note:
        ...
        By default, "timeit()" temporarily turns off _garbage
        collection_ during the timing.  The advantage of this approach
        ...
        re-enabled as the first statement in the _setup_ string.  For
        example:
...
<
   autorange(callback=None)
      ...
      If _callback_ is given and is not "None", it will be called
   ...
   repeat(repeat=5, number=1000000)
      ...
      argument specifies the _number_ argument for "timeit()".
      ...
      Changed in version 3.7: Default value of _repeat_ changed from 3
      to 5.
...
<
      ...
      in the compiled template will be displayed.  The optional _file_
...
pass a _setup_ parameter which contains an import statement:
>
   def test():
...
<
Another option is to pass "globals()" to the  _globals_ parameter,
