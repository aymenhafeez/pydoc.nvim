Python 3.9.25
*parser.pyx*                                  Last change: 2025 Dec 20
...
Warning:
  ...
  versions of Python. For the majority of use cases you can leverage
...
parser.expr(source)
   ...
   The "expr()" function parses the parameter _source_ as if it were
...
parser.suite(source)
   ...
   The "suite()" function parses the parameter _source_ as if it were
...
parser.st2list(st, line_info=False, col_info=False)
   ...
   This function accepts an ST object from the caller in _st_ and
...
parser.compilest(st, filename='<syntax-tree>')
   ...
   _st_ to the parser, using the source file name specified by the
   _filename_ parameter. The default value supplied for _filename_
...
parser.isexpr(st)
   ...
   When _st_ represents an "'eval'" form, this function returns
