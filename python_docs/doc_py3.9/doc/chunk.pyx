Python 3.9.25
*chunk.pyx*                                   Last change: 2025 Dec 20
...
Deprecated since version 3.11: The "chunk" module is deprecated (see
**PEP 594** for details).
...
|           |          | header                          |
+-----------+----------+---------------------------------+
| 8         | _n_      | Data bytes, where _n_ is the    |
|           |          | size given in the preceding     |
|           |          | field                           |
+-----------+----------+---------------------------------+
| 8 + _n_   | 0 or 1   | Pad byte needed if _n_ is odd   |
...
class chunk.Chunk(file, align=True, bigendian=True, inclheader=False)
   ...
   object.  If the optional argument _align_ is true, chunks are
   assumed to be aligned on 2-byte boundaries.  If _align_ is false,
   no alignment is assumed.  The default value is true.  If the
   optional argument _bigendian_ is false, the chunk size is assumed
   to be in little-endian order. This is needed for WAVE audio files.
   The default value is true.  If the optional argument _inclheader_
   ...
   read(size=-1)
      ...
      Read at most _size_ bytes from the chunk (less if the read hits
      the end of the chunk before obtaining _size_ bytes).  If the
      _size_ argument is negative or omitted, read all data until the
