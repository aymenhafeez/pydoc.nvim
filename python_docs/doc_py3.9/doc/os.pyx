...
os.fsencode(filename)
   ...
   Encode _path-like_ _filename_ to the filesystem encoding with
...
os.fsdecode(filename)
   ...
   Decode the _path-like_ _filename_ from the filesystem encoding with
   "'surrogateescape'" error handler, or "'strict'" on Windows; return
   "str" unchanged.
...
os.getenv(key, default=None)
   ...
   Return the value of the environment variable _key_ if it exists, or
   _default_ if it doesn’t. _key_, _default_ and the result are str.
...
os.getenvb(key, default=None)
   ...
   Return the value of the environment variable _key_ if it exists, or
   _default_ if it doesn’t. _key_, _default_ and the result are bytes.
...
os.mkdir(path, mode=0o777, *, dir_fd=None)
   ...
   Changed in version 3.9.20: Windows now handles a _mode_ of "0o700".
...
os.rename(src, dst, *, src_dir_fd=None, dst_dir_fd=None)
   ...
   "OSError" is raised. If both are files, _dst_ it will be replaced
...
os.utime(path, times=None, *[, ns], dir_fd=None, follow_symlinks=True)
   ...
   * If _times_ is "None" and _ns_ is unspecified, this is equivalent
   ...
   object with the _ns_ parameter to _utime_.
...
os.walk(top, topdown=True, onerror=None, followlinks=False)
   ...
   "'.'" and "'..'"). _filenames_ is a list of the names of the non-
   directory files in _dirpath_. Note that the names in the lists
   ...
   the _dirpath_ directory during generating the lists, whether a name
...
os.setxattr(path, attribute, value, flags=0, *, follow_symlinks=True)
   ...
   a str, it is encoded with the filesystem encoding.  _flags_ may be
...
os.posix_spawn(path, argv, env, *, file_actions=None, setpgroup=None, resetids=False, setsid=False, setsigmask=(), setsigdef=(), scheduler=None)
   ...
   for _posix_spawn_. _setsid_ requires "POSIX_SPAWN_SETSID" or
...
os.times()
   ...
   * "elapsed" - elapsed real time since a fixed point in the past
   ...
   See the Unix manual page _times(2)_ and _times(3)_ manual page on
