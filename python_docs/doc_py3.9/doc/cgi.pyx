Python 3.9.25
*cgi.pyx*                                     Last change: 2025 Dec 20
...
Deprecated since version 3.11: The "cgi" module is deprecated (see
...
form contains non-ASCII characters, use the _encoding_ keyword
...
HTML document or by the _Content-Type_ header.  This reads the form
...
for the optional _keep_blank_values_ keyword parameter when creating
the "FieldStorage" instance.

For instance, the following code (which assumes that the _Content-
Type_ header and blank line have already been printed) checks that the
...
multiple files from one field (using a recursive _multipart/*_
encoding). When this occurs, the item will be a dictionary-like
"FieldStorage" item. This can be determined by testing its "type"
attribute, which should be _multipart/form-data_ (or perhaps another
MIME type matching _multipart/*_).  In this case, it can be iterated
over recursively just like the top-level form object.

When a form is submitted in the “old” format (as the query string or
as a single data part of type _application/x-www-form-urlencoded_),
...
cgi.parse(fp=None, environ=os.environ, keep_blank_values=False, strict_parsing=False, separator="&")
   ...
   to "sys.stdin").  The _keep_blank_values_, _strict_parsing_ and
   _separator_ parameters are passed to "urllib.parse.parse_qs()"
   unchanged.

cgi.parse_multipart(fp, pdict, encoding="utf-8", errors="replace", separator="&")

   Parse input of type _multipart/form-data_ (for  file uploads).
   Arguments are _fp_ for the input file, _pdict_ for a dictionary
   containing other parameters in the _Content-Type_ header, and
   _encoding_, the request encoding.
   ...
   Changed in version 3.7: Added the _encoding_ and _errors_
   parameters.  For non-file fields, the value is now a list of
   strings, not bytes.

   Changed in version 3.9.2: Added the _separator_ parameter.

cgi.parse_header(string)

   Parse a MIME header (such as _Content-Type_) into a main value and
