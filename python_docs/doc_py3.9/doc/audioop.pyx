...
Deprecated since version 3.11: The "audioop" module is deprecated (see
**PEP 594** for details).
...
samples 8, 16, 24 or 32 bits wide, stored in _bytes-like objects_.
All scalar items are integers, unless specified otherwise.

Changed in version 3.4: Support for 24-bit samples was added. All
functions now accept any _bytes-like object_. String input now results
...
audioop.findfit(fragment, reference)
   ...
   tuple "(offset, factor)" where _offset_ is the (integer) offset
...
audioop.lin2adpcm(fragment, width, state)
   ...
   "None" can be passed as the state. _adpcmfrag_ is the ADPCM coded
...
should send the _initial_ state (the one you passed to "lin2adpcm()")
