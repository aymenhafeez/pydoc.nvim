...
locale.getpreferredencoding(do_setlocale=True)
   ...
   "'UTF-8'", the locale and the _do_setlocale_ argument are ignored.
...
locale.currency(val, symbol=True, grouping=False, international=False)
   ...
   true, which is the default. If _grouping_ is true (which is not the
   default), grouping is done with the value. If _international_ is
