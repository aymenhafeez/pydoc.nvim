...
str.encode(encoding="utf-8", errors="strict")
   ...
   By default, the _errors_ argument is not checked for best
   performances, but only used at the first encoding error. Enable the
   Python Development Mode, or use a debug build to check _errors_.

   Changed in version 3.1: Support for keyword arguments added.

   Changed in version 3.9: The _errors_ is now checked in development
