...
class pprint.PrettyPrinter(indent=1, width=80, depth=None, stream=None, *, compact=False, sort_dicts=True)
   ...
   _stream_ keyword; the only method used on the stream object is the
   file protocol’s "write()" method.  If not specified, the
   "PrettyPrinter" adopts "sys.stdout".  The amount of indentation
   added for each recursive level is specified by _indent_; the
   ...
   which may be printed is controlled by _depth_; if the data
   ...
   constrained using the _width_ parameter; the default is 80
   characters.  If a structure cannot be formatted within the
   constrained width, a best effort will be made.  If _compact_ is
   false (the default) each item of a long sequence will be formatted
   on a separate line.  If _compact_ is true, as many items as will
   fit within the _width_ will be formatted on each output line. If
   _sort_dicts_ is true (the default), dictionaries will be formatted
...
pprint.pformat(object, indent=1, width=80, depth=None, *, compact=False, sort_dicts=True)
   ...
   _indent_, _width_, _depth_, _compact_ and _sort_dicts_ will be
...
pprint.pprint(object, stream=None, indent=1, width=80, depth=None, *, compact=False, sort_dicts=True)
   ...
   "print = pprint.pprint" for use within a scope).  _indent_,
   _width_, _depth_, _compact_ and _sort_dicts_ will be passed to the
