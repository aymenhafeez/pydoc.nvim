...
whatis expression
   ...
   Print the type of the _expression_.
...
alias [name [command]]
   ...
   Create an alias called _name_ that executes _command_.  The command
   must _not_ be enclosed in quotes.  Replaceable parameters can be
   indicated by "%1", "%2", and so on, while "%*" is replaced by all
   the parameters. If no command is given, the current alias for
   _name_ is shown. If no arguments are given, all aliases are listed.
