...
Changed in version 3.4: _spawn_ added on all unix platforms, and
_forkserver_ added for some unix platforms. Child processes no longer
inherit all of the parents inheritable handles on Windows.

On Unix using the _spawn_ or _forkserver_ start methods will also
...
class multiprocessing.Process(group=None, target=None, name=None, args=(), kwargs={}, *, daemon=None)
   ...
   By default, no arguments are passed to _target_.
...
multiprocessing.set_start_method(method)
   ...
   _method_ can be "'fork'", "'spawn'" or "'forkserver'".
...
class multiprocessing.pool.Pool([processes[, initializer[, initargs[, maxtasksperchild[, context]]]]])
   ...
   New in version 3.2: _maxtasksperchild_

   New in version 3.4: _context_
...
multiprocessing.connection.wait(object_list, timeout=None)
   ...
   For both Unix and Windows, an object can appear in _object_list_ if
   it is
...
* An "'AF_PIPE'" address is a string of the form
  ...
  address of the form "r'\_ServerName_\pipe{PipeName}'" instead.
...
Explicitly pass resources to child processes
   ...
   On Unix using the _fork_ start method, a child process can make use
...
There are a few extra restriction which don’t apply to the _fork_
start method.
