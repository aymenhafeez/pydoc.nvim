...
<
If _source_ is a directory, this will create an archive from the
contents of _source_.  If _source_ is a file, it should be an archive,
...
-o <output>, --output=<output>
   ...
   Write the output to a file named _output_.  If this option is not
   specified, the output filename will be the same as the input
   _source_, with the extension ".pyz" added.  If an explicit filename
   is given, it is used as is (so a ".pyz" extension should be
   included if required).

   An output filename must be specified if the _source_ is an archive
   (and in that case, _output_ must not be the same as _source_).

-p <interpreter>, --python=<interpreter>

   Add a "#!" line to the archive specifying _interpreter_ as the
...
-m <mainfn>, --main=<mainfn>
   ...
   Write a "__main__.py" file to the archive that executes _mainfn_.
   The _mainfn_ argument should have the form “pkg.mod:fn”, where
...
zipapp.create_archive(source, target=None, interpreter=None, main=None, filter=None, compressed=False)
   ...
   Create an application archive from _source_.  The source can be any
   of the following:

   * The name of a directory, or a _path-like object_ referring to a
     directory, in which case a new application archive will be
     created from the content of that directory.

   * The name of an existing application archive file, or a _path-like
     object_ referring to such a file, in which case the file is
     copied to the target (modifying it to reflect the value given for
     the _interpreter_ argument).  The file name should include the
   ...
   * A file object open for reading in bytes mode.  The content of the
   ...
   The _target_ argument determines where the resulting archive will
   be written:

   * If it is the name of a file, or a _path-like object_, the archive
   ...
   * If the target is omitted (or "None"), the source must be a
   ...
   The _interpreter_ argument specifies the name of the Python
   ...
   the Python launcher.  Omitting the _interpreter_ results in no
   ...
   The _main_ argument specifies the name of a callable which will be
   ...
   contain a "__main__.py" file.  The _main_ argument should take the
   ...
   It is an error to omit _main_ if the source is a directory and does
   not contain a "__main__.py" file, as otherwise the resulting
   archive would not be executable.

   The optional _filter_ argument specifies a callback function that
   ...
   The optional _compressed_ argument determines whether files are
   ...
   If a file object is specified for _source_ or _target_, it is the
   ...
   New in version 3.7: Added the _filter_ and _compressed_ arguments.

zipapp.get_interpreter(archive)

   Return the interpreter specified in the "#!" line at the start of
   the archive.  If there is no "#!" line, return "None". The
   _archive_ argument can be a filename or a file-like object open for
...
3. The suggested launcher above uses the Python embedding API.  This
   ...
   application, and _not_ a conventional Python interpreter.  Your
...
2. Standard zipfile data, as generated by the "zipfile" module.  The
   zipfile content _must_ include a file called "__main__.py" (which
