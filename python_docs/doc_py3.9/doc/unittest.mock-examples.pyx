...
mock. Instances are created by _calling the class_. This means you
...
class, you provide a mock of this object that _also_ provides
...
mock, using the _spec_ keyword argument. Accessing methods /
...
use _spec_set_ instead of _spec_.
...
decorated function in the same order they applied (the normal _Python_
...
the module that _uses_ it. See where to patch.
...
Whilst writing tests today I needed to patch an _unbound method_
...
_real_ function object. This function object has the same signature as
...
assertions about the _most recent_ call. If your mock is going to be
called several times, and you want to make assertions about _all_
...
_references_ to the arguments. If the arguments are mutated by the
...
example I’m using _another_ mock to store the arguments so that I can
...
Note:
  ...
  An alternative to using "MagicMock" is to use "Mock" and _only_
  ...
  A _third_ option is to use "MagicMock" but passing in "dict" as the
  _spec_ (or _spec_set_) argument so that the "MagicMock" created only
...
prevent circular dependencies, for which there is _usually_ a much
...
import. Importing fetches an _object_ from the "sys.modules"
dictionary. Note that it fetches an _object_, which need not be a
module. Importing a module for the first time results in a module
object being put in _sys.modules_, so usually when you import
something you get a module back. This need not be the case however.

This means you can use "patch.dict()" to _temporarily_ put a mock in
...
The "Mock" class allows you to track the _order_ of method calls on
...
interested in asserting about _some_ of those calls. You may not even
