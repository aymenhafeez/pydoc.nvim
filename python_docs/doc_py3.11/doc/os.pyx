...
os.copy_file_range(src, dst, count, offset_src=None, offset_dst=None)
   ...
   current position; respectively for _offset_dst_. The files pointed
   by _src_ and _dst_ must reside in the same filesystem, otherwise an
...
os.pidfd_open(pid, flags=0)
   ...
   Return a file descriptor referring to the process _pid_.  This
   descriptor can be used to perform process management without races
   and signals.  The _flags_ argument is provided for future
