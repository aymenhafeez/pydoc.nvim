...
<
This configuration does _almost_ what we want, except that
"sys.stdout" would show messages of severity "ERROR" and above as well
as "INFO" and "WARNING" messages. To prevent this, we can set up a
filter which excludes those messages and add it to the relevant
handler. This can be configured by adding a "filters" section parallel
to "formatters" and "handlers":
>
   {
       "filters": {
           "warnings_and_below": {
...
single file from _multiple processes_ is _not_ supported, because
...
one of the "_XXX_Message" classes.
...
can test them in an environment which has "pynng" installed. Juat for
...
The formatted message _will_ be encoded using UTF-8 encoding by
...
format string and the arguments, not just the format string. That’s
because the __ notation is just syntax sugar for a constructor call to
one of the "_XXX_Message" classes shown above.


Configuring filters with "dictConfig()"
=======================================

You _can_ configure filters using "dictConfig()", though it might not
...
configured by detault to use the older **RFC 3164**, which hails from
...
<
You should do this _after_ configuring logging for your needs. In the
above example, the "basicConfig()" call does this (using the
"sys.stderr" value _before_ it is overwritten by a "LoggerWriter"
...
code which writes to "sys.stderr" makes mutiple writes, each of which
...
patterns which are _unhelpful_, and which should therefore be avoided
...
Opening a file multiple times might _appear_ to work most of the time,
