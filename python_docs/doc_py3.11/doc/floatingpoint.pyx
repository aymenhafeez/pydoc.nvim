...
convert 0.1 to the closest fraction it can of the form _J_/2**_N_
where _J_ is an integer containing exactly 53 bits. Rewriting
>
   1 / 10 ~= J / (2**N)
<
as
...
little bit smaller than 1/10.  But in no case can it be _exactly_
1/10!
