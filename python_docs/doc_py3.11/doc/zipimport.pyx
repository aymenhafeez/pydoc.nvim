...
class zipimport.zipimporter(archivepath)
   ...
   "ZipImportError" is raised if _archivepath_ doesn’t point to a
   valid ZIP archive.
   ...
   find_module(fullname, path=None)
      ...
      Search for a module specified by _fullname_. _fullname_ must be
      the fully qualified (dotted) module name. It returns the
      zipimporter instance itself if the module was found, or "None"
      if it wasn’t. The optional _path_ argument is ignored—it’s there
   ...
   get_data(pathname)
      ...
      Return the data associated with _pathname_. Raise "OSError" if
   ...
   load_module(fullname)
      ...
      Load the module specified by _fullname_. _fullname_ must be the
   ...
   equal the original _archivepath_ argument given to the
