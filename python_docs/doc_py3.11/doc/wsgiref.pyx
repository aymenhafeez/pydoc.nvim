...
wsgiref.util.request_uri(environ, include_query=True)
   ...
   **PEP 3333**.  If _include_query_ is false, the query string is not
...
class wsgiref.util.FileWrapper(filelike, blksize=8192)
   ...
   protocol used to convert a file-like object to an _iterator_. The
   resulting objects are _iterable_s. As the object is iterated over,
   the optional _blksize_ parameter will be repeatedly passed to the
   _filelike_ object’s "read()" method to obtain bytestrings to yield.
...
class wsgiref.headers.Headers([headers])
   ...
   default value of _headers_ is an empty list.
...
wsgiref.simple_server.demo_app(environ, start_response)
   ...
   key/value pairs provided in the _environ_ parameter.  It’s useful
...
class wsgiref.simple_server.WSGIServer(server_address, RequestHandlerClass)
   ...
   "(host,port)" tuple, and _RequestHandlerClass_ should be the
...
class wsgiref.simple_server.WSGIRequestHandler(request, client_address, server)
   ...
   _client_address_ (a "(host,port)" tuple), and _server_
   ("WSGIServer" instance).
   ...
   _handler_class_ to the "make_server()" function.  Some possibly
...
wsgiref.validate.validator(application)
   ...
   **RFC 2616**.
   ...
   warnings will be written to "sys.stderr" (_not_ "wsgi.errors",
...
class wsgiref.handlers.BaseCGIHandler(stdin, stdout, stderr, environ, multithread=True, multiprocess=False)
   ...
   explicitly. The _multithread_ and _multiprocess_ values are used to
...
class wsgiref.handlers.SimpleHandler(stdin, stdout, stderr, environ, multithread=True, multiprocess=False)
   ...
   The "write()" method of _stdout_ should write each chunk in full,
...
class wsgiref.handlers.BaseHandler
   ...
   _write(data)
      ...
      Buffer the bytes _data_ for transmission to the client.  It’s
