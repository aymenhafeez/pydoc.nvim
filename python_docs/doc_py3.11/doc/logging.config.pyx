...
logging.config.fileConfig(fname, defaults=None, disable_existing_loggers=True, encoding=None)
   ...
   Parameters:
      ...
      * **encoding** – The encoding used to open file when _fname_ is
        filename.
   ...
   Changed in version 3.11.4: An exception will be thrown if the
...
configuration files from untrusted sources with _extreme caution_ and
...
* _filters_ - the corresponding value will be a dict in which each key
...
* _loggers_ - the corresponding value will be a dict in which each key
...
* _disable_existing_loggers_ - whether any existing non-root loggers
...
import callable on a configurator _instance_.
