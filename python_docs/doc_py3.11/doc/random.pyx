...
random.randrange(start, stop[, step])
   ...
   Deprecated since version 3.10: The automatic conversion of non-
   integer types to equivalent integers is deprecated.  Currently
   "randrange(10.0)" is losslessly converted to "randrange(10)".  In
   the future, this will raise a "TypeError".

   Deprecated since version 3.10: The exception raised for non-integer
   values such as "randrange(10.5)" or "randrange('10')" will be
...
random.sample(population, k, *, counts=None)
   ...
   Changed in version 3.11: The _population_ must be a sequence.
...
random.gammavariate(alpha, beta)
   ...
   Gamma distribution.  (_Not_ the gamma function!)  The shape and
   scale parameters, _alpha_ and _beta_, must have positive values.
...
random.gauss(mu=0.0, sigma=1.0)
   ...
   Normal distribution, also called the Gaussian distribution. _mu_ is
   the mean, and _sigma_ is the standard deviation.  This is slightly
   ...
   Changed in version 3.11: _mu_ and _sigma_ now have default
   arguments.
...
class random.Random([seed])
   ...
   Changed in version 3.11: Formerly the _seed_ could be any hashable
