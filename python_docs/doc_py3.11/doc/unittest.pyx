...
Changed in version 3.11: "unittest" dropped the _namespace packages_
...
class unittest.TestLoader
   ...
   loadTestsFromModule(module, pattern=None)
      ...
      _use_load_tests_ default argument is deprecated and ignored,
      although it is still accepted for backward compatibility.  The
      method also now accepts a keyword-only argument _pattern_ which
...
class unittest.TextTestRunner(stream=None, descriptions=True, verbosity=1, failfast=False, buffer=False, resultclass=None, warnings=None, *, tb_locals=False)
   ...
   options (see Warning control) and leaving _warnings_ to "None".
