...
test.support.unlink(filename)
   ...
   Call "os.unlink()" on _filename_.  On Windows platforms, this is
...
test.support.match_test(test)
   ...
   Match _test_ to patterns set in "set_match_tests()".

test.support.set_match_tests(patterns)

   Define match test with regular expression _patterns_.
...
test.support.strip_python_strerr(stderr)
   ...
   Strip the _stderr_ of a Python process from potential debug output
...
test.support.start_threads(threads, unlock=None)
   ...
   Context manager to start _threads_.  It attempts to join the
   threads upon exit.
...
@test.support.bigaddrspacetest(f)
   ...
   Decorator for tests that fill the address space.  _f_ is the
   function to wrap.
...
test.support.check_free_after_iterating(test, iter, cls, args=())
   ...
   Assert that _iter_ is deallocated after iterating.
...
test.support.check__all__(test_case, module, name_of_module=None, extra=(), blacklist=())
   ...
   The _blacklist_ argument can be a set of names that must not be
...
class test.support.BasicTestRunner
   ...
   run(test)
      ...
      Run _test_ and return the result.
...
test.support.script_helper.assert_python_ok(*args, **env_vars)
   ...
   If the "__cleanenv" keyword is set, _env_vars_ is used as a fresh
   environment.
