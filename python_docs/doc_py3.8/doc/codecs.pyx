...
but some module features are restricted to use specifically with _text
encodings_, or with codecs that encode to "bytes".
...
The following error handlers are only applicable to _text encodings_:
...
codecs.replace_errors(exception)
   ...
   Implements the "'replace'" error handling (for _text encodings_
...
codecs.xmlcharrefreplace_errors(exception)
   ...
   with _text encodings_ only): the unencodable character is replaced
   by an appropriate XML character reference.

codecs.backslashreplace_errors(exception)

   Implements the "'backslashreplace'" error handling (for _text
   encodings_ only): malformed data is replaced by a backslashed
   escape sequence.

codecs.namereplace_errors(exception)

   Implements the "'namereplace'" error handling (for encoding with
   _text encodings_ only): the unencodable character is replaced by a
   "\N{...}" escape sequence.
...
use the third-party _idna module <https://pypi.org/project/idna/>__.
