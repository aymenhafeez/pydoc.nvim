...
py_compile.compile(file, cfile=None, dfile=None, doraise=False, optimize=-1, invalidation_mode=PycInvalidationMode.TIMESTAMP, quiet=0)
   ...
   _dfile_ is specified, it is used as the name of the source file in
   error messages instead of _file_.  If _doraise_ is true, a
   "PyCompileError" is raised when an error is encountered while
   compiling _file_. If _doraise_ is false (the default), an error
...
py_compile.main(args=None)
   ...
   command line, if _args_ is "None") are compiled and the resulting
