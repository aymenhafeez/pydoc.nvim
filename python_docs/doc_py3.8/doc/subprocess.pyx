...
subprocess.run(args, *, stdin=None, input=None, stdout=None, stderr=None, capture_output=False, shell=False, cwd=None, timeout=None, check=False, encoding=None, errors=None, text=None, env=None, universal_newlines=None, **other_popen_kwargs)
   ...
   with "stdout=PIPE" and "stderr=PIPE". The _stdout_ and _stderr_
   arguments may not be supplied at the same time as _capture_output_.
   If you wish to capture and combine both streams into one, use
   "stdout=PIPE" and "stderr=STDOUT" instead of _capture_output_.

   The _timeout_ argument is passed to "Popen.communicate()". If the
   ...
   When used, the internal "Popen" object is automatically created
   with "stdin=PIPE", and the _stdin_ argument may not be used as
   well.
...
<
   ...
   Changed in version 3.8.17: Changed Windows shell search order for
...
commonly needed are:
   ...
   parent.  Additionally, _stderr_ can be "STDOUT", which indicates
   that the stderr data from the child process should be captured into
   the same file handle as for _stdout_.
   ...
   New in version 3.6: Added _encoding_ and _errors_ parameters.

   New in version 3.7: Added the _text_ parameter as an alias for
...
<
   ...
   * negative bufsize (the default) means the system default of
   ...
   standard input, standard output and standard error file handles,
   respectively.  Valid values are "PIPE", "DEVNULL", an existing file
   descriptor (a positive integer), an existing _file object_, and
   "None".  "PIPE" indicates that a new pipe to the child should be
   created.  "DEVNULL" indicates that the special file "os.devnull"
   will be used. With the default settings of "None", no redirection
   will occur; the child’s file handles will be inherited from the
   ...
   Note:
     ...
     _start_new_session_ parameter can take the place of a previously
     common use of _preexec_fn_ to call os.setsid() in the child.
   ...
   or _path-like_ object.  In particular, the function looks for
   ...
   with the specified encoding and _errors_, as described above in
   ...
   passed to the underlying "CreateProcess" function. _creationflags_,
...
Popen.stdin
   ...
   _errors_ arguments were specified or the _universal_newlines_
   argument was "True", the stream is a text stream, otherwise it is a
   byte stream. If the _stdin_ argument was not "PIPE", this attribute
   is "None".
...
Popen.stdout
   ...
   _errors_ arguments were specified or the _universal_newlines_
   argument was "True", the stream is a text stream, otherwise it is a
   byte stream. If the _stdout_ argument was not "PIPE", this
   attribute is "None".
...
Popen.stderr
   ...
   _errors_ arguments were specified or the _universal_newlines_
   argument was "True", the stream is a text stream, otherwise it is a
   byte stream. If the _stderr_ argument was not "PIPE", this
   attribute is "None".
