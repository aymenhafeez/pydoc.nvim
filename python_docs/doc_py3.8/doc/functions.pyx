...
abs(x)
   ...
   number, its magnitude is returned. If _x_ defines "__abs__()",
...
compile(source, filename, mode, flags=0, dont_inherit=False, optimize=-1)
   ...
   future statements affect the compilation of _source_.  If neither
   ...
   "compile()".  If the _flags_ argument is given and _dont_inherit_
   is not (or is zero) then the future statements specified by the
   _flags_ argument are used in addition to those that would be used
   anyway. If _dont_inherit_ is a non-zero integer then the _flags_
   ...
   The optional argument _flags_ also controls whether the compiled
...
eval(expression[, globals[, locals]])
   ...
   _expression_ is parsed.  This means that _expression_ normally has
   ...
   omitted, the expression is executed with the _globals_ and _locals_
   in the environment where "eval()" is called.  Note, _eval()_ does
   not have access to the _nested scopes_ (non-locals) in the
...
exec(object[, globals[, locals]])
   ...
   same dictionary. If exec gets two separate objects as _globals_ and
...
class int(x, base=10)
   ...
   literal in radix _base_.  Optionally, the literal can be preceded
   ...
   _base_ is 10. The allowed values are 0 and 2–36. Base-2, -8, and
...
isinstance(object, classinfo)
   ...
   _object_ is an instance of any of the types. If _classinfo_ is not
...
iter(object[, sentinel])
   ...
   argument, _sentinel_, is given, then _object_ must be a callable
   object.  The iterator created in this case will call _object_ with
   no arguments for each call to its "__next__()" method; if the value
   returned is equal to _sentinel_, "StopIteration" will be raised,
...
open(file, mode='r', buffering=-1, encoding=None, errors=None, newline=None, closefd=True, opener=None)
   ...
   buffer.  When no _buffering_ argument is given, the default
   ...
   * "'namereplace'" (also only supported when writing) replaces
   ...
   _newline_ controls how _universal newlines_ mode works (it only
...
print(*objects, sep=' ', end='\n', file=sys.stdout, flush=False)
   ...
   Whether output is buffered is usually determined by _file_, but if
   the _flush_ keyword argument is true, the stream is forcibly
   flushed.
...
zip(*iterables)
   ...
   Returns an iterator of tuples, where the _i_-th tuple contains the
   _i_-th element from each of the argument sequences or iterables.
...
<
   ...
   n-length groups using "zip(*[iter(s)]*n)".  This repeats the _same_
