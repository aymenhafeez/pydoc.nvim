...
shutil.copy(src, dst, *, follow_symlinks=True)
   ...
   filename from _src_.  Returns the path to the newly created file.
...
shutil.copytree(src, dst, symlinks=False, ignore=None, copy_function=copy2, ignore_dangling_symlinks=False, dirs_exist_ok=False)
   ...
   directory named _dst_ and return the destination directory.
   _dirs_exist_ok_ dictates whether to raise an exception in case
   _dst_ or any missing parent directory already exists.
   ...
   _ignore_dangling_symlinks_ argument to silent dangling symlinks
...
shutil.unpack_archive(filename[, extract_dir[, format[, filter]]])
   ...
   3.8.17, is passed to the underlying unpacking function. For zip
   ...
   Changed in version 3.8.17: Added the _filter_ argument.
