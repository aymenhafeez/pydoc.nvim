...
POP_FINALLY(preserve_tos)
   ...
   Cleans up the value stack and the block stack.  If _preserve_tos_
...
BUILD_TUPLE_UNPACK(count)
   ...
   Pops _count_ iterables from the stack, joins them in a single
...
BUILD_MAP_UNPACK(count)
   ...
   Pops _count_ mappings from the stack, merges them into a single
...
CALL_FINALLY(delta)
   ...
   increments bytecode counter by _delta_.  Used for calling the
