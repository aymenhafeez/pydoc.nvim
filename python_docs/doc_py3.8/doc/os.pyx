...
os.environ
   ...
   A _mapping_ object representing the string environment. For
...
os.environb
   ...
   Bytes version of "environ": a _mapping_ object representing the
...
os.setgroups(groups)
   ...
   Note:
     ...
     On Mac OS X, the length of _groups_ may not exceed the system-
...
os.sendfile(out, in, offset, count[, headers][, trailers], flags=0)
   ...
   Copy _count_ bytes from file descriptor _in_ to file descriptor
   _out_ starting at _offset_. Return the number of bytes sent. When
   ...
   the current position of _in_ and the position of _in_ is updated.

   The second case may be used on Mac OS X and FreeBSD where _headers_
   and _trailers_ are arbitrary sequences of buffers that are written
   before and after the data from _in_ is written. It returns the same
   as the first case.

   On Mac OS X and FreeBSD, a value of 0 for _count_ specifies to send
   until the end of _in_ is reached.

   All platforms support sockets as _out_ file descriptor, and some
...
os.mkdir(path, mode=0o777, *, dir_fd=None)
   ...
   Changed in version 3.8.20: Windows now handles a _mode_ of "0o700".
...
os.replace(src, dst, *, src_dir_fd=None, dst_dir_fd=None)
   ...
   directory, "OSError" will be raised.  If _dst_ exists and is a
   file, it will be replaced silently if the user has permission.  The
   operation may fail if _src_ and _dst_ are on different filesystems.
   If successful, the renaming will be an atomic operation (this is a
   POSIX requirement).
...
os.waitid(idtype, id, options)
   ...
   can be "P_PID", "P_PGID" or "P_ALL". _id_ specifies the pid to wait
   on. _options_ is constructed from the ORing of one or more of
...
os.sched_setparam(pid, param)
   ...
   Set a scheduling parameters for the process with PID _pid_. A _pid_
   of 0 means the calling process. _param_ is a "sched_param"
   instance.
...
os.urandom(size)
   ...
   Return a string of _size_ random bytes suitable for cryptographic
   use.
