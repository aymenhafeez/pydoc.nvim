...
treat the cursor as an _iterator_, call the cursor’s "fetchone()"
...
sqlite3.connect(database[, timeout, detect_types, isolation_level, check_same_thread, factory, cached_statements, uri])
   ...
   **converters** registered with the module-level
   ...
   If _uri_ is true, _database_ is interpreted as a URI. This allows
...
class sqlite3.Connection
   ...
   execute(sql[, parameters])
      ...
      method with the _parameters_ given, and returns the cursor.

   executemany(sql[, parameters])

      This is a nonstandard shortcut that creates a cursor object by
      calling the "cursor()" method, calls the cursor’s
      "executemany()" method with the _parameters_ given, and returns
      the cursor.
   ...
   executescript(sql_script)
      ...
      This is a nonstandard shortcut that creates a cursor object by
      calling the "cursor()" method, calls the cursor’s
      "executescript()" method with the given _sql_script_, and
      returns the cursor.
...
<
   ...
   executemany(sql, seq_of_parameters)
      ...
      mappings found in the sequence _seq_of_parameters_.  The
      "sqlite3" module also allows using an _iterator_ yielding
