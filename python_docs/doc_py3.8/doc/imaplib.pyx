...
class imaplib.IMAP4_SSL(host='', port=IMAP4_SSL_PORT, keyfile=None, certfile=None, ssl_context=None)
   ...
   _keyfile_ and _certfile_ are a legacy alternative to _ssl_context_
   - they can point to PEM-formatted private key and certificate chain
   files for the SSL connection.  Note that the _keyfile_/_certfile_
   parameters are mutually exclusive with _ssl_context_, a
   "ValueError" is raised if _keyfile_/_certfile_ is provided along
   with _ssl_context_.

   Changed in version 3.3: _ssl_context_ parameter added.
   ...
   deprecated in favor of _ssl_context_. Please use
