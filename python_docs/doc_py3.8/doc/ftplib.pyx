...
FTP.retrlines(cmd, callback=None)
   ...
   Retrieve a file or directory listing in ASCII transfer mode.  _cmd_
   ...
   those files. "NLST" retrieves a list of file names. The _callback_
   function is called for each line with a string argument containing
   the line with the trailing CRLF stripped.  The default _callback_
...
FTP.storlines(cmd, fp, callback=None)
   ...
   Store a file in ASCII transfer mode.  _cmd_ should be an
   appropriate "STOR" command (see "storbinary()").  Lines are read
   until EOF from the _file object_ _fp_ (opened in binary mode) using
