...
class email.mime.application.MIMEApplication(_data, _subtype='octet-stream', _encoder=email.encoders.encode_base64, *, policy=compat32, **_params)
   ...
   __data_ is a string containing the raw byte data.  Optional
...
class email.mime.audio.MIMEAudio(_audiodata, _subtype=None, _encoder=email.encoders.encode_base64, *, policy=compat32, **_params)
   ...
   create MIME message objects of major type _audio_. __audiodata_ is
   a string containing the raw audio data.  If this data can be
   decoded by the standard Python module "sndhdr", then the subtype
   will be automatically included in the _Content-Type_ header.
   Otherwise you can explicitly specify the audio subtype via the
   __subtype_ argument.  If the minor type could not be guessed and
   __subtype_ was not given, then "TypeError" is raised.
...
class email.mime.image.MIMEImage(_imagedata, _subtype=None, _encoder=email.encoders.encode_base64, *, policy=compat32, **_params)
   ...
   create MIME message objects of major type _image_. __imagedata_ is
