...
gettext.bindtextdomain(domain, localedir=None)
   ...
   "_localedir_/_language_/LC_MESSAGES/_domain_.mo", where _language_
...
gettext.bind_textdomain_codeset(domain, codeset=None)
   ...
   Bind the _domain_ to _codeset_, changing the encoding of byte
   strings returned by the "lgettext()", "ldgettext()", "lngettext()"
   and "ldngettext()" functions. If _codeset_ is omitted, then the
...
gettext.ngettext(singular, plural, n)
   ...
   found, apply the plural formula to _n_, and return the resulting
   message (some languages have more than two plural forms). If no
   translation is found, return _singular_ if _n_ is 1; return
   _plural_ otherwise.
...
gettext.find(domain, localedir=None, languages=None, all=False)
   ...
   It takes a _domain_, identical to what "textdomain()" takes.
   Optional _localedir_ is as in "bindtextdomain()". Optional
   _languages_ is a list of strings, where each string is a language
   code.

   If _localedir_ is not given, then the default system locale
   directory is used. [2]  If _languages_ is not given, then the
   ...
   such file is found, then "None" is returned. If _all_ is given, it
...
gettext.translation(domain, localedir=None, languages=None, class_=None, fallback=False, codeset=None)
   ...
   _localedir_, and _languages_, which are first passed to "find()" to
   get a list of the associated ".mo" file paths.  Instances with
   identical ".mo" file names are cached.  The actual class
   instantiated is _class__ if provided, otherwise "GNUTranslations".
   The class’s constructor must take a single _file object_ argument.
   If provided, _codeset_ will change the charset used to encode
   ...
   _fallback_ is false (which is the default), and returns a
   "NullTranslations" instance if _fallback_ is true.
...
class gettext.NullTranslations(fp=None)
   ...
   Takes an optional _file object_ _fp_, which is ignored by the base
   class. Initializes “protected” instance variables __info_ and
   __charset_ which are set by derived classes, as well as
   __fallback_, which is set through "add_fallback()".  It then calls
   "self._parse(fp)" if _fp_ is not "None".

   _parse(fp)

      No-op in the base class, this method takes file object _fp_, and
   ...
   gettext(message)
      ...
      Otherwise, return _message_.  Overridden in derived classes.

   ngettext(singular, plural, n)

      If a fallback has been set, forward "ngettext()" to the
      fallback. Otherwise, return _singular_ if _n_ is 1; return
      _plural_ otherwise. Overridden in derived classes.
...
class gettext.GNUTranslations
   ...
   gettext(message)
      ...
      no entry in the catalog for the _message_ id, and a fallback has
   ...
   ngettext(singular, plural, n)
      ...
      the message id for purposes of lookup in the catalog, while _n_
      ...
      "ngettext()" method.  Otherwise, when _n_ is 1 _singular_ is
...
<
   pgettext(context, message)
      ...
      is no entry in the catalog for the _message_ id and _context_,
   ...
   npgettext(context, singular, plural, n)
      ...
      If the message id for _context_ is not found in the catalog, and
      a fallback is specified, the request is forwarded to the
      fallback’s "npgettext()" method.  Otherwise, when _n_ is 1
...
[1] The default locale directory is system dependent; for example, on
    ...
    "_sys.base_prefix_/share/locale" (see "sys.base_prefix"). For this
