...
* For "AF_INET6" address family, a four-tuple "(host, port, flowinfo,
  scopeid)" is used, where _flowinfo_ and _scopeid_ represent the
  "sin6_flowinfo" and "sin6_scope_id" members in "struct sockaddr_in6"
  in C.  For "socket" module methods, _flowinfo_ and _scopeid_ can be
  omitted just for backward compatibility.  Note, however, omission of
  _scopeid_ can cause problems in manipulating scoped IPv6 addresses.

  Changed in version 3.7: For multicast addresses (with _scopeid_
  meaningful) _address_ may not contain "%scope" (or "zone id") part.
...
socket.socket(family=AF_INET, type=SOCK_STREAM, proto=0, fileno=None)
   ...
   system _socket()_ call.  Therefore,
...
socket.create_server(address, *, family=AF_INET, backlog=None, reuse_port=False, dualstack_ipv6=False)
   ...
   reasonable value is chosen. _reuse_port_ dictates whether to set
...
socket.getnameinfo(sockaddr, flags)
   ...
   _sockaddr_ contains meaningful _scopeid_. Usually this happens for
   multicast addresses.
...
socket.ntohs(x)
   ...
   Deprecated since version 3.7: In case _x_ does not fit in 16-bit
...
socket.recvfrom(bufsize[, flags])
   ...
   _address_ does not contain "%scope" part anymore. In order to get
