Python 3.8.20
*formatter.pyx*                               Last change: 2025 Dec 20
...
implementations: The _formatter_ interface, and the _writer_ interface
...
formatter.pop_style(n=1)
   ...
   Pop the last _n_ style specifications passed to "push_style()".  A
...
class formatter.NullFormatter(writer=None)
   ...
   A formatter which does nothing.  If _writer_ is omitted, a
...
writer.new_styles(styles)
   ...
   values; the value "AS_IS" should be ignored.  The _styles_ tuple
...
writer.send_label_data(data)
   ...
   Set _data_ to the left of the current left margin, if possible. The
   value of _data_ is not restricted; treatment of non-string values
...
class formatter.DumbWriter(file=None, maxcol=72)
   ...
   Simple writer class which writes output on the _file object_ passed
