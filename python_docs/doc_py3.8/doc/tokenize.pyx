...
the _named tuple_ returned from "tokenize.tokenize()".


Tokenizing Input
================

The primary entry point is a _generator_:

tokenize.tokenize(readline)

   The "tokenize()" generator requires one argument, _readline_, which
   ...
   found. The line passed (the last tuple item) is the _physical_
   line.  The 5 tuple is returned as a _named tuple_ with the field
   names: "type string start end line".

   The returned _named tuple_ has an additional property named
...
tokenize.generate_tokens(readline)
   ...
   Like "tokenize()", the _readline_ argument is a callable returning
   a single line of input. However, "generate_tokens()" expects
   _readline_ to return a str object rather than bytes.
