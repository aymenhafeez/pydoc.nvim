...
class cmd.Cmd(completekey='tab', stdin=None, stdout=None)
   ...
   The optional argument _completekey_ is the "readline" name of a
   completion key; it defaults to "Tab". If _completekey_ is not
   ...
   instance’s "use_rawinput" attribute to "False", otherwise _stdin_
   will be ignored.
...
Cmd.cmdloop(intro=None)
   ...
   value. The _stop_ argument to "postcmd()" is the return value from
...
Cmd.onecmd(str)
   ...
   there is a "do_*()" method for the command _str_, the return value
...
Cmd.precmd(line)
   ...
   may re-write the command or simply return _line_ unchanged.

Cmd.postcmd(stop, line)

   Hook method executed just after a command dispatch is finished.
   This method is a stub in "Cmd"; it exists to be overridden by
   subclasses.  _line_ is the command line which was executed, and
   _stop_ is a flag which indicates whether execution will be
   ...
   corresponds to _stop_; returning false will cause interpretation to
   continue.
