...
|===============|===================================|=========|
| "x or y"      | if _x_ is false, then _y_, else   | (1)     |
|               | _x_                               |         |
+---------------+-----------------------------------+---------+
...
int.to_bytes(length, byteorder, *, signed=False)
   ...
   The integer is represented using _length_ bytes.  An
   ...
   represent the integer.  If _byteorder_ is ""big"", the most
   significant byte is at the beginning of the byte array.  If
   _byteorder_ is ""little"", the most significant byte is at the end
...
class str(object=b'', encoding='utf-8', errors='strict')
   ...
   string representation of _object_.  For string objects, this is the
   string itself.  If _object_ does not have a "__str__()" method,
...
str.encode(encoding="utf-8", errors="strict")
   ...
   encoding is "'utf-8'". _errors_ may be given to set a different
   error handling scheme. The default for _errors_ is "'strict'",
...
class bytes([source[, encoding[, errors]]])
   ...
   * Copying existing binary data via the buffer protocol:
   ...
   hex([sep[, bytes_per_sep]])
      ...
      _bytes_per_sep_ parameter controls the spacing.  Positive values
...
bytearray.decode(encoding="utf-8", errors="strict")
   ...
   "'utf-8'". _errors_ may be given to set a different error handling
   scheme.  The default for _errors_ is "'strict'", meaning that
...
class memoryview(obj)
   ...
   Create a "memoryview" that references _obj_.  _obj_ must support
   ...
   memory unit handled by the originating object _obj_.  For many
...
called _instance method_) object. When called, it will add the "self"
