...
ipaddress.ip_address(address)
   ...
   default. A "ValueError" is raised if _address_ does not represent a
...
ipaddress.ip_network(address, strict=True)
   ...
   supplied; integers less than 2**32 will be considered to be IPv4 by
   default.  _strict_ is passed to "IPv4Network" or "IPv6Network"
...
ipaddress.ip_interface(address)
   ...
   IPv4 by default.  A "ValueError" is raised if _address_ does not
...
<
      ...
      Changed in version 3.8.20: Fixed some false positives and false
      negatives.
