...
At the highest level, the transport is concerned with _how_ bytes are
transmitted, while the protocol determines _which_ bytes to transmit
...
"loop.create_connection()") usually accept a _protocol_factory_
argument used to create a _Protocol_ object for an accepted
connection, represented by a _Transport_ object. Such methods usually
...
class asyncio.WriteTransport(BaseTransport)
   ...
   Instances of the _WriteTransport_ class are returned from the
...
class asyncio.ReadTransport(BaseTransport)
   ...
   Instances of the _ReadTransport_ class are returned from the
...
class asyncio.Transport(WriteTransport, ReadTransport)
   ...
   Instances of the _Transport_ class are returned from or used by
...
class asyncio.DatagramTransport(BaseTransport)
   ...
   Instances of the _DatagramTransport_ class are returned from the
...
class asyncio.SubprocessTransport(BaseTransport)
   ...
   Instances of the _SubprocessTransport_ class are returned from
...
BaseTransport.get_extra_info(name, default=None)
   ...
   _name_ is a string representing the piece of transport-specific
   information to get.

   _default_ is the value to return if the information is not
...
WriteTransport.get_write_buffer_limits()
   ...
   Get the _high_ and _low_ watermarks for write flow control. Return
   a tuple "(low, high)" where _low_ and _high_ are positive number of
   bytes.
...
WriteTransport.set_write_buffer_limits(high=None, low=None)
   ...
   Neither _high_ nor _low_ can be negative.

   "pause_writing()" is called when the buffer size becomes greater
   than or equal to the _high_ value. If writing has been paused,
   "resume_writing()" is called when the buffer size becomes less than
   or equal to the _low_ value.
   ...
   watermark.  Setting _high_ to zero forces _low_ to zero as well,
   and causes "pause_writing()" to be called whenever the buffer
   becomes non-empty.  Setting _low_ to zero causes "resume_writing()"
...
DatagramTransport.sendto(data, addr=None)
   ...
   Send the _data_ bytes to the remote peer given by _addr_ (a
   transport-dependent target address).  If _addr_ is "None", the data
...
SubprocessTransport.get_pipe_transport(fd)
   ...
   the integer file descriptor _fd_:

   * "0": readable streaming transport of the standard input
     (_stdin_), or "None" if the subprocess was not created with
     "stdin=PIPE"

   * "1": writable streaming transport of the standard output
     (_stdout_), or "None" if the subprocess was not created with
     "stdout=PIPE"

   * "2": writable streaming transport of the standard error
     (_stderr_), or "None" if the subprocess was not created with
     "stderr=PIPE"
...
SubprocessTransport.send_signal(signal)
   ...
   Send the _signal_ number to the subprocess, as in
...
SubprocessTransport.close()
   ...
   _stdin_, _stdout_, and _stderr_ pipes.
...
BaseProtocol.connection_made(transport)
   ...
   The _transport_ argument is the transport representing the
...
Protocol.data_received(data)
   ...
   Called when some data is received.  _data_ is a non-empty bytes
   ...
   _eof_received()_ is called, "data_received()" is not called
   anymore.
...
Python 3.7 _on a provisional basis_!  This is as an experimental API
...
BufferedProtocol.get_buffer(sizehint)
   ...
   _sizehint_ is the recommended minimum size for the returned buffer.
   It is acceptable to return smaller or larger buffers than what
   _sizehint_ suggests.  When set to -1, the buffer size can be
...
BufferedProtocol.buffer_updated(nbytes)
   ...
   _nbytes_ is the total number of bytes that were written to the
   buffer.
...
DatagramProtocol.datagram_received(data, addr)
   ...
   Called when a datagram is received.  _data_ is a bytes object
   containing the incoming data.  _addr_ is the address of the peer
   sending the data; the exact format depends on the transport.

DatagramProtocol.error_received(exc)

   Called when a previous send or receive operation raises an
   "OSError".  _exc_ is the "OSError" instance.
