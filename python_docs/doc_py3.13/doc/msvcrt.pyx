...
msvcrt.locking(fd, mode, nbytes)
   ...
   extends from the current file position for _nbytes_ bytes, and may
   continue beyond the end of the file. _mode_ must be one of the
...
msvcrt.set_error_mode(mode)
   ...
   for an error that might end the program. _mode_ must be one of the
...
msvcrt.CrtSetReportMode(type, mode)
   ...
   type generated by "_CrtDbgReport()" in the MS VC++ runtime. _type_
   must be one of the "CRT_*" constants listed below. _mode_ must be
...
msvcrt.CrtSetReportFile(type, file)
   ...
   you can specify the file handle to receive the message text. _type_
   must be one of the "CRT_*" constants listed below. _file_ should be
