...
sys.monitoring.free_tool_id(tool_id: int, /) -> None
   ...
   Should be called once a tool no longer requires _tool_id_.

Note:

  "free_tool_id()" will not disable global or local events associated
  with _tool_id_, nor will it unregister any callback functions. This
  function is only intended to be used to notify the VM that the
  particular _tool_id_ is no longer in use.
...
<
  If there are no arguments, _arg0_ is set to
  "sys.monitoring.MISSING".
