Python 3.13.11
*howto-curses.pyx*                            Last change: 2025 Dec 20
...
the curses library will figure out what control codes need to be sent
to the terminal to produce the right output.  curses doesn’t provide
many user-interface concepts such as buttons, checkboxes, or dialogs;
if you need such features, consider a user interface library such as
Urwid.
...
of the terminal.  The callable is called inside a "try"…"except" that
catches exceptions, restores the state of the terminal, and then re-
raises the exception.  Therefore your terminal won’t be left in a
...
are always passed in the order _y,x_, and the top-left corner of a
window is coordinate (0,0).  This breaks the normal convention for
handling coordinates where the _x_ coordinate comes first.  This is an
...
"curses.LINES" and "curses.COLS" variables to obtain the _y_ and _x_
...
| Form                              | Description                                     |
|===================================|=================================================|
| _str_ or _ch_                     | Display the string _str_ or character _ch_ at   |
|                                   | the current position                            |
+-----------------------------------+-------------------------------------------------+
| _str_ or _ch_, _attr_             | Display the string _str_ or character _ch_,     |
|                                   | using attribute _attr_ at the current position  |
+-----------------------------------+-------------------------------------------------+
...
so if you leave out the _y,x_ coordinates, the string or character
...
synonym for "curs_set()".  When _bool_ is true, the curses library
...
color pair _n_, to foreground color f and background color b.  Color
...
* “Use curses… don’t swear”: video of a PyCon 2013 talk on controlling
  terminals using curses or Urwid.
