...
class multiprocessing.shared_memory.SharedMemory(name=None, create=False, size=0, *, track=True)
   ...
   Parameters:
      * **name** (_str__ | __None_) – The unique name for the
      ...
      * **create** (_bool_) – Control whether a new shared memory
        block is created ("True") or an existing shared memory block
        is attached ("False").

      * **size** (_int_) – The requested number of bytes when creating
        a new shared memory block. Because some platforms choose to
        allocate chunks of memory based upon that platform’s memory
        page size, the exact size of the shared memory block may be
        larger or equal to the size requested. When attaching to an
        existing shared memory block, the _size_ parameter is ignored.

      * **track** (_bool_) – When "True", register the shared memory
        ...
        accessing shared memory with _track_ enabled. This will cause
        the shared memory to be deleted by the resource tracker of the
        first process that terminates. To avoid this issue, users of
        "subprocess" or standalone Python processes should set _track_
        to "False" when there is already another process in place that
        does the bookkeeping. _track_ is ignored on Windows, which has
...
class multiprocessing.managers.SharedMemoryManager([address[, authkey]])
   ...
   Refer to "BaseManager" for a description of the inherited _address_
   and _authkey_ optional input arguments and how they may be used to
   ...
   ShareableList(sequence)
      ...
      the values from the input _sequence_.
...
class multiprocessing.shared_memory.ShareableList(sequence=None, *, name=None)
   ...
   * "None"
   ...
   _sequence_ is used in populating a new "ShareableList" full of
   values. Set to "None" to instead attach to an already existing
   "ShareableList" by its unique shared memory name.

   _name_ is the unique name for the requested shared memory, as
   ...
   unique name while leaving _sequence_ set to "None".

   Note:

     A known issue exists for "bytes" and "str" values. If they end
     with "\x00" nul bytes or characters, those may be _silently
     stripped_ when fetching them by index from the "ShareableList".
...
<
   ...
   index(value)
      ...
      Return first index position of _value_. Raise "ValueError" if
      _value_ is not present.
