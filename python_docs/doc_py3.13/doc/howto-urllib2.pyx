...
easy to read. This HOWTO aims to illustrate using _urllib_, with
...
Second, you can pass extra information (“metadata”) _about_ the data
...
the HTTP standard makes it clear that POSTs are intended to _always_
cause side-effects, and GET requests _never_ to cause side-effects,
...
_urlopen_ raises "URLError" when it cannot handle a response (though
...
So if you want to be prepared for "HTTPError" _or_ "URLError" there
...
Note:
  ...
  The "except HTTPError" _must_ come first, otherwise "except
...
Note:
  ...
  Currently "urllib.request" _does not_ support fetching of "https"
...
[5] In my case I have to use a proxy to access the internet at work.
    If you attempt to fetch _localhost_ URLs through this proxy it
