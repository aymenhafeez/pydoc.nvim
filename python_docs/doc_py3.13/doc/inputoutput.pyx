Python 3.13.11
*inputoutput.pyx*                             Last change: 2025 Dec 20
...
"format % values" (where _format_ is a string), "%" conversion
specifications in _format_ are replaced with zero or more elements of
_values_. This operation is commonly known as string interpolation.
For example:
>
   >>> import math
...
_encoding_. If _encoding_ is not specified, the default is platform
...
the file in _binary mode_. Binary mode data is read and written as
"bytes" objects. You can not specify _encoding_ when opening file in
binary mode.
...
object (in binary mode). _size_ is an optional numeric argument.  When
_size_ is omitted or negative, the entire contents of the file will be
read and returned; it’s your problem if the file is twice as large as
your machine’s memory. Otherwise, at most _size_ characters (in text
mode) or _size_ bytes (in binary mode) are read and returned. If the
...
The position is computed from adding _offset_ to a reference point;
the reference point is selected by the _whence_ argument.  A _whence_
...
_whence_ can be omitted and defaults to 0, using the beginning of the
...
valid _offset_ values are those returned from the "f.tell()", or zero.
Any other _offset_ value produces undefined behaviour.
...
_serializing_.  Reconstructing the data from the string representation
is called _deserializing_.  Between serializing and deserializing, the
...
serializes the object to a _text file_.  So if "f" is a _text file_
object opened for writing, we can do this:
>
   json.dump(x, f)
<
To decode the object again, if "f" is a _binary file_ or _text file_
...
Note:
  ...
  opening JSON file as a _text file_ for both of reading and writing.
...
See also:
  ...
  Contrary to JSON, _pickle_ is a protocol which allows the
