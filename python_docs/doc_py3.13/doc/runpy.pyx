...
Note that this is _not_ a sandbox module - all code is executed in the
...
runpy.run_module(mod_name, init_globals=None, run_name=None, alter_sys=False)
   ...
   The _mod_name_ argument should be an absolute module name. If the
   ...
   The optional dictionary argument _init_globals_ may be used to pre-
   populate the module’s globals dictionary before the code is
   executed. _init_globals_ will not be modified. If any of the
   special global variables below are defined in _init_globals_, those
   ...
   package and to the _mod_name_ argument otherwise.

   "__spec__" will be set appropriately for the _actually_ imported
   module (that is, "__spec__.name" will always be _mod_name_ or
   "mod_name + '.__main__'", never _run_name_).

   "__file__", "__cached__", "__loader__" and "__package__" are set as
   normal based on the module spec.

   If the argument _alter_sys_ is supplied and evaluates to "True",
...
runpy.run_path(path_name, init_globals=None, run_name=None)
   ...
   supplied to the CPython command line, _file_path_ may refer to a
   ...
   If _file_path_ directly references a script file (whether as source
   or as precompiled byte code), then "__file__" will be set to
   _file_path_, and "__spec__", "__cached__", "__loader__" and
   "__package__" will all be set to "None".

   If _file_path_ is a reference to a valid "sys.path" entry, then
   ...
   updated with the value of _file_path_ and "sys.modules[__name__]"
