...
dbm.whichdb(filename)
   ...
   database modules available — "dbm.sqlite3", "dbm.gnu", "dbm.ndbm",
...
mutable _mappings_; keys and their corresponding values can be stored,
...
dbm.sqlite3.open(filename, /, flag='r', mode=0o666)
   ...
   Parameters:
      * **filename** (_path-like object_) – The path to the database
        to be opened.
   ...
   _mapping_, but the "keys()" method returns a list, and the
...
dbm.gnu.open_flags
   ...
   A string of characters the _flag_ parameter of "open()" supports.
...
dbm.gnu.open(filename, flag='r', mode=0o666, /)
   ...
   "gdbm" objects behave similar to mutable _mappings_, but methods
...
dbm.ndbm.open(filename, flag='r', mode=0o666, /)
   ...
   "ndbm" objects behave similar to mutable _mappings_, but methods
   "items()", "values()", "pop()", "popitem()", and "update()" are not
   supported, the "keys()" method returns a list, and the
   "setdefault()" method requires two arguments. It also supports a
   “closing” context manager via the "with" keyword.
...
dbm.dumb.open(filename, flag='c', mode=0o666)
   ...
   The returned database object behaves similar to a mutable
   _mapping_, but the "keys()" and "items()" methods return lists, and
