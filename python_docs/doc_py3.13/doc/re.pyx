...
"*?", "+?", "??"
   ...
   the quantifier makes it perform the match in _non-greedy_ or
...
"*+", "++", "?+"
   ...
   _possessive_ quantifiers. For example, "a*a" will match "'aaaa'"
...
"{m,n}+"
   Causes the resulting RE to match from _m_ to _n_ repetitions of the
   preceding RE, attempting to match as many repetitions as possible
   _without_ establishing any backtracking points. This is the
...
"(?aiLmsux)"
   ...
   * "re.X" (verbose)
   ...
   instead of passing a _flag_ argument to the "re.compile()"
...
"(?>...)"
   ...
   can only be unwound to a point _before_ the "(?>...)" because once
   exited, the expression, known as an _atomic group_, has thrown away
...
"(?P<name>...)"
   ...
   Changed in version 3.12: In "bytes" patterns, group _name_ can only
...
"(?(id/name)yes-pattern|no-pattern)"
   ...
   Changed in version 3.12: Group _id_ can only contain ASCII digits.
   In "bytes" patterns, group _name_ can only contain bytes in the
...
Changed in version 3.8: The "'\N{_name_}'" escape sequence has been
...
re.match(pattern, string, flags=0)
   ...
   regular expression _pattern_, return a corresponding "Match".
...
<
   ...
   Deprecated since version 3.13: Passing _maxsplit_ and _flags_ as
...
<
   ...
   occurrence of _pattern_.  The function takes a single "Match"
...
<
   ...
   Changed in version 3.12: Group _id_ can only contain ASCII digits.
   In "bytes" replacement strings, group _name_ can only contain bytes
   in the ASCII range ("b'\x00'"-"b'\x7f'").

   Deprecated since version 3.13: Passing _count_ and _flags_ as
   positional arguments is deprecated. In future Python versions they
   will be keyword-only parameters.
