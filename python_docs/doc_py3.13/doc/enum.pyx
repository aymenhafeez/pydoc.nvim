...
class enum.IntEnum
   ...
   _IntEnum_ is the same as "Enum", but its members are also integers
...
class enum.StrEnum
   ...
   _StrEnum_ is the same as "Enum", but its members are also strings
...
>
      ...
      3
<
   If a "Flag" operation is performed with an _IntFlag_ member and:
...
class enum.EnumDict
   ...
   _EnumDict_ is a subclass of "dict" that is used as the namespace
