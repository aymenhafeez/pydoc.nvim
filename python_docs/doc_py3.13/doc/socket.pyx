...
* "AF_BLUETOOTH" supports the following protocols and address formats:
  ...
  * "BTPROTO_HCI" accepts a format that depends on your OS.
    ...
    * On Linux it accepts a tuple "(device_id,)" where "device_id" is
      an integer specifying the number of the Bluetooth device.
  ...
  * "BTPROTO_SCO" accepts "bdaddr" where "bdaddr" is the Bluetooth
    ...
    "'12:23:34:45:56:67'" or "b'12:23:34:45:56:67'") This protocol is
...
* "AF_PACKET" is a low-level interface directly to network devices.
  ...
  * _proto_ - The Ethernet protocol number. May be "ETH_P_ALL" to
...
socket.ETH_P_ALL
   ...
   "ETH_P_ALL" can be used in the "socket" constructor as _proto_ for
   the "AF_PACKET" family in order to capture every packet, regardless
   of protocol.

   For more information, see the _packet(7)_ manpage.
...
socket.HCI_DATA_DIR
   ...
   For use with "BTPROTO_HCI". "HCI_FILTER" is only available on Linux
   and FreeBSD. "HCI_TIME_STAMP" and "HCI_DATA_DIR" are only available
   on Linux.
...
socket.create_connection(address, timeout=GLOBAL_DEFAULT, source_address=None, *, all_errors=False)
   ...
   _all_errors_ is "True", it is an "ExceptionGroup" containing the
...
socket.recv(bufsize[, flags])
   ...
   received at once is specified by _bufsize_. A returned empty bytes
...
socket.setsockopt(level, optname, None, optlen: int)
   ...
   _setsockopt(2)_).  The needed symbolic constants are defined in
   this module (_SO_* etc. <socket-unix-constants>_).  The value can
   be an integer, "None" or a _bytes-like object_ representing a
   ...
   When _value_ is set to "None", _optlen_ argument is required. It’s
