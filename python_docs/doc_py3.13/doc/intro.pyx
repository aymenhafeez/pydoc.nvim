...
sections of this manual, it helps to start with chapter Built-in
...
* If an availability note contains both a minimum Kernel version and a
  ...
  feature with note _Availability: Linux >= 3.17 with glibc >= 2.27_
...
* Mobile apps have limited access to modify system resources (such as
  the system clock). These resources will often be _readable_, but
...
* Console input and output:
  ...
  * iOS apps have a limited concept of console output. "stdout" and
    "stderr" _exist_, and content written to "stdout" and "stderr"
    will be visible in logs when running in Xcode, but this content
    _won’t_ be recorded in the system log. If a user who has installed
