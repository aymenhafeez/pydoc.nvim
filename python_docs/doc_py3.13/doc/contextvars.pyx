...
_Context Variables_.  The "copy_context()" function and the "Context"
...
<
   The required _name_ parameter is used for introspection and debug
   purposes.

   The optional keyword-only _default_ parameter is returned by
   ...
   **Important:** Context Variables should be created at the top
   ...
   get([default])
      ...
      * return the value of the _default_ argument of the method, if
        provided; or
   ...
   set(value)
      ...
      The required _value_ argument is the new value for the context
      variable.
   ...
   reset(token)
      ...
      "ContextVar.set()" that created the _token_ was used.
...
class contextvars.Token
   ...
   _Token_ objects are returned by the "ContextVar.set()" method. They
...
<
   The function has an _O_(1) complexity, i.e. works equally fast for
...
class contextvars.Context
   ...
   _current context_ is the "Context" object at the top of the current
   thread’s stack.  All "Context" objects in the stacks are considered
   to be _entered_.

   _Entering_ a context, which can be done by calling its "run()"
   method, makes the context the current context by pushing it onto
   the top of the current thread’s context stack.

   _Exiting_ from the current context, which can be done by returning
   ...
   run(callable, *args, **kwargs)
      ...
      exits the Context.  Returns _callable_’s return value, or
...
<
   ...
   var in context
      ...
      Return "True" if the _context_ has a value for _var_ set; return
      "False" otherwise.

   context[var]

      Return the value of the _var_ "ContextVar" variable. If the
      variable is not set in the context object, a "KeyError" is
      raised.

   get(var[, default])

      Return the value for _var_ if _var_ has the value in the context
      object.  Return _default_ otherwise.  If _default_ is not given,
      return "None".
