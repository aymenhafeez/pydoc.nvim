Python 3.13.11
*shelve.pyx*                                  Last change: 2025 Dec 20
...
shelve.open(filename, flag='c', protocol=None, writeback=False)
   ...
   interpretation as the _flag_ parameter of "dbm.open()".
   ...
   specified with the _protocol_ parameter.

   Because of Python semantics, a shelf cannot know when a mutable
   persistent-dictionary entry is modified.  By default modified
   objects are written _only_ when assigned to the shelf (see
   Example).  If the optional _writeback_ parameter is set to "True",
...
Shelf.sync()
   ...
   _writeback_ set to "True".  Also empty the cache and synchronize
...
* The "shelve" module does not support _concurrent_ read/write access
...
class shelve.Shelf(dict, protocol=None, writeback=False, keyencoding='utf-8')
   ...
   By default, pickles created with "pickle.DEFAULT_PROTOCOL" are used
   to serialize values.  The version of the pickle protocol can be
   specified with the _protocol_ parameter.  See the "pickle"
   ...
   The _keyencoding_ parameter is the encoding used to encode keys
   ...
   Changed in version 3.2: Added the _keyencoding_ parameter;
...
class shelve.BsdDbShelf(dict, protocol=None, writeback=False, keyencoding='utf-8')
   ...
   other database modules.  The _dict_ object passed to the
   constructor must support those methods.  This is generally
   accomplished by calling one of "bsddb.hashopen()", "bsddb.btopen()"
   or "bsddb.rnopen()".  The optional _protocol_, _writeback_, and
   _keyencoding_ parameters have the same interpretation as for the
   "Shelf" class.

class shelve.DbfilenameShelf(filename, flag='c', protocol=None, writeback=False)

   A subclass of "Shelf" which accepts a _filename_ instead of a dict-
   like object.  The underlying file will be opened using
   "dbm.open()".  By default, the file will be created and opened for
   both read and write.  The optional _flag_ parameter has the same
   interpretation as for the "open()" function.  The optional
   _protocol_ and _writeback_ parameters have the same interpretation
