...
"cProfile" and "profile" provide _deterministic profiling_ of Python
programs. A _profile_ is a set of statistics that describes how often
...
calls, 207 were _primitive_, meaning that the call was not induced via
...
cumtime
   ...
   invocation till exit). This figure is accurate _even_ for recursive
   functions.
...
profile.runctx(command, globals, locals, filename=None, sort=-1)
   ...
   the globals and locals mappings for the _command_ string. This
   routine executes:
>
...
<
   ...
   print_stats(sort=-1)
      ...
      The _sort_ parameter specifies the sorting order of the
...
class pstats.Stats(*filenames or profile, stream=sys.stdout)
   ...
   specific, there is _no_ file compatibility guaranteed with future
   ...
   dump_stats(filename)
      ...
      _filename_.  The file is created if it does not exist, and is
...
contrast, _statistical profiling_ (which is not done by this module)
...
deterministic profiling. Python automatically provides a _hook_
...
accuracy of the clock (less than one clock tick), but it _can_
...
_only_ appear if you have calibrated your profiler, and the results
