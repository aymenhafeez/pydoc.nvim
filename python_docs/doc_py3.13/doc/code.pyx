Python 3.13.11
*code.pyx*                                    Last change: 2025 Dec 20
...
class code.InteractiveInterpreter(locals=None)
   ...
   The optional _locals_ argument specifies a mapping to use as the
   ...
   specified by _locals_. They are only pickleable if _locals_ is the
...
class code.InteractiveConsole(locals=None, filename='<console>', local_exit=False)
   ...
   _local_exit_ is true, "exit()" and "quit()" in the console will not
...
code.interact(banner=None, readfunc=None, local=None, exitmsg=None, local_exit=False)
   ...
   new instance of "InteractiveConsole" and sets _readfunc_ to be used
   as the "InteractiveConsole.raw_input()" method, if provided.  If
   _local_ is provided, it is passed to the "InteractiveConsole"
   constructor for use as the default namespace for the interpreter
   loop.  If _local_exit_ is provided, it is passed to the
   "InteractiveConsole" constructor.  The "interact()" method of the
   instance is then run with _banner_ and _exitmsg_ passed as the
...
InteractiveInterpreter.runsource(source, filename='<input>', symbol='single')
   ...
   same as for "compile_command()"; the default for _filename_ is
   "'<input>'", and for _symbol_ is "'single'".  One of several things
   can happen:
...
InteractiveConsole.interact(banner=None, exitmsg=None)
   ...
   _banner_ argument specify the banner to print before the first
   ...
   The optional _exitmsg_ argument specifies an exit message printed
   when exiting. Pass the empty string to suppress the exit message.
   If _exitmsg_ is not given or "None", a default message is printed.
