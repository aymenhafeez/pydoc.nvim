...
class mmap.mmap(fileno, length, flags=MAP_SHARED, prot=PROT_WRITE | PROT_READ, access=ACCESS_DEFAULT, offset=0, *, trackfd=True)
   ...
   **(Unix version)** Maps _length_ bytes from the file specified by
   ...
   If _trackfd_ is "False", the file descriptor specified by _fileno_
...
<
   ...
   resize(newsize)
      ...
      Resizing a map created with _access_ of "ACCESS_READ" or
      "ACCESS_COPY", will raise a "TypeError" exception. Resizing a
      map created with _trackfd_ set to "False", will raise a
