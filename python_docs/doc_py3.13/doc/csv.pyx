...
class csv.DictReader(f, fieldnames=None, restkey=None, restval=None, dialect='excel', *args, **kwds)
   ...
   If the argument passed to _fieldnames_ is an iterator, it will be
   coerced to a "list".
...
csv.QUOTE_NONE
   ...
   _delimiter_, _quotechar_, _escapechar_, "'\r'", "'\n'" or any of
   the characters in _lineterminator_ occurs in output data it is
   preceded by the current _escapechar_ character. If _escapechar_ is
   not set, the writer will raise "Error" if any characters that
   require escaping are encountered. Set _quotechar_ to "None" to
...
Dialect.escapechar
   ...
   require escaping:
      ...
      * the _delimiter_, the _quotechar_, "'\r'", "'\n'" and any of
        the characters in _lineterminator_ are escaped if _quoting_ is
        set to "QUOTE_NONE";

      * the _quotechar_ is escaped if _doublequote_ is "False";

      * the _escapechar_ itself.

   On reading, the _escapechar_ removes any special meaning from the
...
Dialect.quotechar
   ...
   characters, such as the _delimiter_ or the _quotechar_, or which
   contain new-line characters ("'\r'", "'\n'" or any of the
   characters in _lineterminator_). It defaults to "'"'". Can be set
   to "None" to prevent escaping "'"'" if _quoting_ is set to
   "QUOTE_NONE".
...
Dialect.quoting
   ...
   constants and defaults to "QUOTE_MINIMAL" if _quotechar_ is not
