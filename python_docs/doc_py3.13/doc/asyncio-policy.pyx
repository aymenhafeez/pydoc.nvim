...
The policy object gets and sets a separate event loop per _context_.
This is per-thread by default, though custom policies could define
_context_ differently.
...
class asyncio.ThreadedChildWatcher
   ...
   children (_O_(1) each time a child terminates), but starting a
...
class asyncio.SafeChildWatcher
   ...
   same _O_(_n_) complexity but requires a running event loop in the
   main thread to work.
...
class asyncio.FastChildWatcher
   ...
   There is no noticeable overhead when handling a big number of
   children (_O_(1) each time a child terminates).
