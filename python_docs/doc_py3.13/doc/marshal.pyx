...
dictionaries, and code objects (if _allow_code_ is true), where it
...
format _version_ lower than 3, recursive lists, sets and dictionaries
...
marshal.dump(value, file, version=version, /, *, allow_code=True)
   ...
   type.  The file must be a writeable _binary file_.
   ...
   back by "load()". Code objects are only supported if _allow_code_
   is true.

   The _version_ argument indicates the data format that "dump" should
   use (see below).
...
marshal.load(file, /, *, allow_code=True)
   ...
   "TypeError". Code objects are only supported if _allow_code_ is
   true. The file must be a readable _binary file_.
...
marshal.dumps(value, version=version, /, *, allow_code=True)
   ...
   _allow_code_ is true.

   The _version_ argument indicates the data format that "dumps"
...
marshal.loads(bytes, /, *, allow_code=True)
   ...
   Convert the _bytes-like object_ to a value.  If no valid value is
   found, raise "EOFError", "ValueError" or "TypeError". Code objects
   are only supported if _allow_code_ is true. Extra bytes in the
   input are ignored.
