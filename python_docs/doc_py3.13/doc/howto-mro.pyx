Python 3.13.11
*howto-mro.pyx*                               Last change: 2025 Dec 20
...
Note:
  ...
  was _introduced_ in Python 2.3, but it is still used in later
  versions – including Python 3.

By Michele Simionato.

Abstract:
   _This document is intended for Python programmers who want to
...
Disclaimer:
   ...
   _I donate this document to the Python Software Foundation, under
   the Python 2.3 license.  As usual in these circumstances, I warn
   the reader that what follows_ should _be correct, but I don’t give
   any warranty.  Use it at your own risk and peril!_

Acknowledgments:

   _All the people of the Python mailing list who sent me their
   ...
   added this document to the official Python 2.3 home-page._
...
applies to the _new style classes_ introduced in Python 2.2:  _classic
classes_ maintain their old method resolution order, depth first and
...
2. The list of the ancestors of a class C, including the class itself,
   ...
   class precedence list or the _linearization_ of C.

3. The _Method Resolution Order_ (MRO) is the set of rules that
...
4. For instance, in the case of single inheritance hierarchy, if C is
   ...
   construct a linearization that respects _local precedence ordering_
   and _monotonicity_.
...
5. I will discuss the local precedence ordering later, but I can give
   ...
   following is true:  _if C1 precedes C2 in the linearization of C,
   then C1 precedes C2 in the linearization of any subclass of C_.
...
exception, but chooses an _ad hoc_ ordering (CABXYO in this case).
...
the linearization L[C] of the class C. The rule is the following:

   _the linearization of C is the sum of C plus the merge of the
   linearizations of the parents and the list of the parents._
...
following prescription:
   ...
   _take the head of the first list, i.e L[B1][0]; if this head is not
   ...
   create the class C and will raise an exception._

This prescription ensures that the merge operation _preserves_ the
...
A MRO is _bad_ when it breaks such fundamental properties as local
...
with inheritance diagram
      ...
      (buy spam)   F
            ...
            (buy eggs or spam ?)
<
We see that class G inherits from F and E, with F _before_ E:
therefore we would expect the attribute _G.remember2buy_ to be
inherited by _F.remember2buy_ and not by _E.remember2buy_:
...
<
It is clear that this linearization is _wrong_, since A comes before D
whereas in the linearization of K3 A comes _after_ D. In other words,
...
_follows_ K3.  These problems explain why the 2.2 rule has been
...
together (and _not_ separately) deserve a prize:  the prize is a short
...
[2] The paper _A Monotonic Superclass Linearization for Dylan_:
    https://doi.org/10.1145/236337.236343

[3] Guido van Rossum’s essay, _Unifying types and classes in Python
    2.2_: https://web.archive.org/web/20140210194412/http://www.python
    .org/download/releases/2.2.2/descrintro
