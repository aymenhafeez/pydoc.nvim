...
pieces of data – _cookies_ – to be set on the client machine by an
...
Note:
  ...
  The various named parameters found in _Set-Cookie_ and _Set-Cookie2_
  headers (eg. "domain" and "expires") are conventionally referred to
  as _attributes_.  To distinguish them from Python attributes, the
  documentation for this module uses the term _cookie-attribute_
  instead.
...
class http.cookiejar.CookieJar(policy=None)
   ...
   _policy_ is an object implementing the "CookiePolicy" interface.
...
class http.cookiejar.FileCookieJar(filename=None, delayload=None, policy=None)
   ...
   Changed in version 3.8: The filename parameter supports a _path-
   like object_.
...
class http.cookiejar.DefaultCookiePolicy(blocked_domains=None, allowed_domains=None, netscape=True, rfc2965=False, rfc2109_as_netscape=None, hide_cookie2=False, strict_domain=False, strict_rfc2965_unverifiable=True, strict_ns_unverifiable=False, strict_ns_domain=DefaultCookiePolicy.DomainLiberal, strict_ns_set_initial_dollar=False, strict_ns_set_path=False, secure_protocols=('https', 'wss'))
   ...
   accept and return cookies. _secure_protocols_ is a sequence of
   protocols for which secure cookies can be added to. By default
   _https_ and _wss_ (secure websocket) are considered secure
   ...
   cookies (ie. cookies received in a _Set-Cookie_ header with a
...
See also:
  ...
  **RFC 2109** - HTTP State Management Mechanism
     Obsoleted by **RFC 2965**. Uses _Set-Cookie_ with version=1.
...
"CookieJar" objects support the _iterator_ protocol for iterating over
...
CookieJar.add_cookie_header(request)
   ...
   respectively), the _Cookie2_ header is also added when appropriate.

   The _request_ object (usually a "urllib.request.Request" instance)
   ...
   Changed in version 3.3: _request_ object needs "origin_req_host"
   attribute. Dependency on a deprecated method
   "get_origin_req_host()" has been removed.

CookieJar.extract_cookies(response, request)

   Extract cookies from HTTP _response_ and store them in the
   "CookieJar", where allowed by policy.

   The "CookieJar" will look for allowable _Set-Cookie_ and _Set-
   Cookie2_ headers in the _response_ argument, and store cookies as
   appropriate (subject to the "CookiePolicy.set_ok()" method’s
   approval).

   The _response_ object (usually the result of a call to
...
CookieJar.make_cookies(response, request)
   ...
   Return sequence of "Cookie" objects extracted from _response_
   object.

   See the documentation for "extract_cookies()" for the interfaces
   required of the _response_ and _request_ arguments.
...
CookiePolicy.set_ok(cookie, request)
   ...
   _cookie_ is a "Cookie" instance.  _request_ is an object
...
CookiePolicy.domain_return_ok(domain, request)
   ...
   Note that "domain_return_ok()" is called for every _cookie_ domain,
   not just for the _request_ domain.  For example, the function might
   be called with both "".example.com"" and ""www.example.com"" if the
   request domain is ""www.example.com"".  The same goes for
   "path_return_ok()".

   The _request_ argument is as documented for "return_ok()".
...
CookiePolicy.hide_cookie2
   ...
   Don’t add _Cookie2_ header to requests (the presence of this header
...
the _blocked_domains_ constructor argument, and "blocked_domains()"
and "set_blocked_domains()" methods (and the corresponding argument
and methods for _allowed_domains_).  If you set an allowlist, you can
...
DefaultCookiePolicy.is_blocked(domain)
   ...
   Return "True" if _domain_ is on the blocklist for setting or
   receiving cookies.
...
DefaultCookiePolicy.is_not_allowed(domain)
   ...
   Return "True" if _domain_ is not on the allowlist for setting or
   receiving cookies.
...
DefaultCookiePolicy.rfc2109_as_netscape
   ...
   2109** cookies (ie. cookies received in a _Set-Cookie_ header with
...
Cookie.rfc2109
   ...
   the cookie arrived in a _Set-Cookie_ header, and the value of the
...
Cookie.port_specified
   ...
   server (in the _Set-Cookie_ / _Set-Cookie2_ header).
