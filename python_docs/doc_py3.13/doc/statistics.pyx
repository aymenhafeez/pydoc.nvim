...
statistics.kde(data, h, kernel='normal', *, cumulative=False)
   ...
   The degree of smoothing is controlled by the scaling parameter _h_
   which is called the bandwidth.  Smaller values emphasize local
   features while larger values give smoother results.

   The _kernel_ determines the relative weights of the sample data
   ...
   _normal_ (_gauss_), _logistic_, and _sigmoid_.

   Kernels that only give weight to sample points within the bandwidth
   include _rectangular_ (_uniform_), _triangular_, _parabolic_
   (_epanechnikov_), _quartic_ (_biweight_), _triweight_, and
   _cosine_.

   If _cumulative_ is true, will return a cumulative distribution
   function.

   A "StatisticsError" will be raised if the _data_ sequence is empty.
...
statistics.kde_random(data, h, kernel='normal', *, seed=None)
   ...
   Providing a _seed_ allows reproducible selections. In the future,
...
statistics.quantiles(data, *, n=4, method='exclusive')
   ...
   larger than _n_. Raises "StatisticsError" if there is not at least
   one data point.
