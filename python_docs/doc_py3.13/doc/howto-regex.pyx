...
_metacharacters_, and don’t match themselves.  Instead, they signal
...
_complementing_ the set.  This is indicated by including a "'^'" as
...
+--------+-------------+-----------------------------------+
| 3      | _Failure_   | The engine tries to match "b",    |
...
The most complicated quantifier is "{m,n}", where _m_ and _n_ are
decimal integers.  This quantifier means there must be at least _m_
...
The simplest case "{m}" matches the preceding item exactly _m_ times.
...
>
   ...
   re.compile('ab*')
<
"re.compile()" also accepts an optional _flags_ argument, used to
...
assertions_.  They don’t cause the engine to advance through the
...
"|"
   Alternation, or the “or” operator.   If _A_ and _B_ are regular
   expressions, "A|B" will match any string that matches either _A_ or
   _B_. "|" has very low precedence in order to make it work
...
this way — but you’ll soon find out that they’re _very_ useful when
...
.sub(replacement, string[, count=0])
   ...
   overlapping occurrences of the RE in _string_ by the replacement
   _replacement_.  If the pattern isn’t found, _string_ is returned
   unchanged.
...
start at 0; if the match wouldn’t start at zero,  "match()" will _not_
report it.
>
