...
Both "list.sort()" and "sorted()" have a _key_ parameter to specify a
...
<
The value of the _key_ parameter should be a function (or other
...
above, or they can be instances of "dataclass" or a _named tuple_.


Operator Module Functions and Partial Function Evaluation
=========================================================

The _key function_ patterns shown above are very common, so Python
...
_grade_ and then ascending _age_, do the _age_ sort first and then
sort again using _grade_:
>
...
* "heapq.nsmallest()" and "heapq.nlargest()" return the _n_ smallest
  and largest values, respectively.  These functions make a single
  pass over the data keeping only _n_ elements in memory at a time.
  For values of _n_ that are small relative to the number of inputs,
