...
commonly needed are:
   ...
   _file object_ with a valid file descriptor.  With the default
   ...
   Additionally, _stderr_ can be "STDOUT", which indicates that the
   stderr data from the child process should be captured into the same
   file handle as for _stdout_.
...
Popen.communicate(input=None, timeout=None)
   ...
   _input_ to a subsequent post-timeout "communicate()" call is in
...
subprocess.STARTF_FORCEONFEEDBACK
   ...
   A "STARTUPINFO.dwFlags" parameter to specify that the _Working in
   Background_ mouse cursor will be displayed while a process is
...
subprocess.getstatusoutput(cmd, *, encoding=None, errors=None)
   ...
   Execute the string _cmd_ in a shell with "check_output()" and
