...
class contextlib.AbstractContextManager
   ...
   An _abstract base class_ for classes that implement
...
<
   The function being decorated must return a _generator_-iterator
...
<
   ...
   Note:
     ...
     Most types managing resources support the _context manager_
     protocol, which closes _thing_ on leaving the "with" statement.
...
contextlib.aclosing(thing)
   ...
   _thing_ upon completion of the block.  This is basically equivalent
   to:
>
      from contextlib import asynccontextmanager
...
contextlib.nullcontext(enter_result=None)
   ...
   Return a context manager that returns _enter_result_ from
...
contextlib.redirect_stdout(new_target)
   ...
   _sys.stdout_. You can capture that output in a string by
...
>
      with open('help.txt', 'w') as f:
          with redirect_stdout(f):
              help(pow)
<
   To send the output of "help()" to _sys.stderr_:
>
...
<
   ...
   that callbacks are _not_ invoked implicitly when the context stack
   ...
   enter_context(cm)
      ...
      "AttributeError" if _cm_ is not a context manager.
   ...
   push(exit)
      ...
      As "__enter__" is _not_ invoked, this method can be used to
...
class contextlib.AsyncExitStack
   ...
   async enter_async_context(cm)
      ...
      Changed in version 3.11: Raises "TypeError" instead of
      "AttributeError" if _cm_ is not an asynchronous context manager.
...
method implementation, _without_ inadvertently catching exceptions
...
also be used _inside_ a "with" statement that is already using the
...
Note also that being reentrant is _not_ the same thing as being thread
...
"ExitStack", as it invokes _all_ currently registered callbacks when
