...
class html.parser.HTMLParser(*, convert_charrefs=True, scripting=False)
   ...
   Changed in version 3.13.10: Added the _scripting_ parameter.
...
HTMLParser.feed(data)
   ...
   more data is fed or "close()" is called.  _data_ must be "str".
...
HTMLParser.handle_charref(name)
   ...
   character references of the form "&#_NNN_;" and "&#x_NNN_;".  For
...
HTMLParser.handle_pi(data)
   ...
   Note:
     ...
     the trailing "'?'" will cause the "'?'" to be included in _data_.
