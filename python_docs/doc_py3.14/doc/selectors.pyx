...
"fileno()" method, or a raw file descriptor. See _file object_.
...
>
   ...
   +-- KqueueSelector
<
In the following, _events_ is a bitwise mask indicating which I/O
...
class selectors.BaseSelector
   ...
   abstractmethod register(fileobj, events, data=None)
      ...
      _fileobj_ is the file object to monitor.  It may either be an
      integer file descriptor or an object with a "fileno()" method.
      _events_ is a bitwise mask of events to monitor. _data_ is an
      opaque object.
   ...
   abstractmethod unregister(fileobj)
      ...
      "KeyError" if _fileobj_ is not registered.  It will raise
      "ValueError" if _fileobj_ is invalid (e.g. it has no "fileno()"
   ...
   abstractmethod select(timeout=None)
      ...
      report the currently ready file objects. If _timeout_ is "None",
      the call will block until a monitored file object becomes ready.

      This returns a list of "(key, events)" tuples, one for each
      ready file object.

      _key_ is the "SelectorKey" instance corresponding to a ready
      file object. _events_ is a bitmask of events ready on this file
      object.
