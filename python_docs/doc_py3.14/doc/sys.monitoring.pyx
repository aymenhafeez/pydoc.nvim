...
sys.monitoring.use_tool_id(tool_id: int, name: str, /) -> None
   ...
   Must be called before _tool_id_ can be used. _tool_id_ must be in
   the range 0 to 5 inclusive. Raises a "ValueError" if _tool_id_ is
   in use.
...
sys.monitoring.free_tool_id(tool_id: int, /) -> None
   ...
   Should be called once a tool no longer requires _tool_id_. Will
   call "clear_tool_id()" before releasing _tool_id_.

sys.monitoring.get_tool(tool_id: int, /) -> str | None

   Returns the name of the tool if _tool_id_ is in use, otherwise it
   returns "None". _tool_id_ must be in the range 0 to 5 inclusive.
...
sys.monitoring.set_events(tool_id: int, event_set: int, /) -> None
   ...
   Activates all events which are set in _event_set_. Raises a
   "ValueError" if _tool_id_ is not in use.
...
sys.monitoring.set_local_events(tool_id: int, code: CodeType, event_set: int, /) -> None
   ...
   Activates all the local events for _code_ which are set in
   _event_set_. Raises a "ValueError" if _tool_id_ is not in use.
...
sys.monitoring.register_callback(tool_id: int, event: int, func: Callable | None, /) -> Callable | None
   ...
   Registers the callable _func_ for the _event_ with the given
   _tool_id_

   If another callback was registered for the given _tool_id_ and
   _event_, it is unregistered and returned. Otherwise
...
<
* "CALL", "C_RAISE" and "C_RETURN" (_arg0_ can be "MISSING"
  specifically):
>
     func(code: CodeType, instruction_offset: int, callable: object, arg0: object) -> object
<
  _code_ represents the code object where the call is being made,
  while _callable_ is the object that is about to be called (and thus
  triggered the event). If there are no arguments, _arg0_ is set to
  "sys.monitoring.MISSING".

  For instance methods, _callable_ will be the function object as
  found on the class with _arg0_ set to the instance (i.e. the "self"
...
<
  Note that the _destination_offset_ is where the code will next
  execute.
