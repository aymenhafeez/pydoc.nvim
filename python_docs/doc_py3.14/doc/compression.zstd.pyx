Python 3.14.2
*compression.zstd.pyx*                        Last change: 2025 Dec 20
...
decompressing data using the Zstandard (or _zstd_) compression
...
This is an _optional module_. If it is missing from your copy of
...
compression.zstd.open(file, /, mode='rb', *, level=None, options=None, zstd_dict=None, encoding=None, errors=None, newline=None)
   ...
   The _file_ argument can be either a file name (given as a "str",
   "bytes" or _path-like_ object), in which case the named file is
   ...
   When reading, the _options_ argument can be a dictionary providing
   advanced decompression parameters; see "DecompressionParameter" for
   detailed information about supported parameters. The _zstd_dict_
   argument is a "ZstdDict" instance to be used during decompression.
   When reading, if the _level_ argument is not None, a "TypeError"
   will be raised.

   When writing, the _options_ argument can be a dictionary providing
   advanced decompression parameters; see "CompressionParameter" for
   detailed information about supported parameters. The _level_
   argument is the compression level to use when writing compressed
   data. Only one of _level_ or _options_ may be non-None. The
   _zstd_dict_ argument is a "ZstdDict" instance to be used during
   compression.

   In binary mode, this function is equivalent to the "ZstdFile"
   constructor: "ZstdFile(file, mode, ...)". In this case, the
   _encoding_, _errors_, and _newline_ parameters must not be
   provided.
...
class compression.zstd.ZstdFile(file, /, mode='rb', *, level=None, options=None, zstd_dict=None)
   ...
   A "ZstdFile" can wrap an already-open _file object_, or operate
   directly on a named file. The _file_ argument specifies either the
   file object to wrap, or the name of the file to open (as a "str",
   "bytes" or _path-like_ object). If wrapping an existing file
   object, the wrapped file will not be closed when the "ZstdFile" is
   closed.

   The _mode_ argument can be either "'rb'" for reading (default),
   ...
   If _file_ is a file object (rather than an actual file name), a
   ...
   detailed information about supported parameters. The _level_
   argument is the compression level to use when writing compressed
   data. Only one of _level_ or _options_ may be passed. The
   _zstd_dict_ argument is a "ZstdDict" instance to be used during
   compression.
   ...
   peek(size=-1)
      ...
      Note:
        ...
        passing a file object for _file_).
   ...
   name
      ...
      attribute of the underlying _file object_.
...
compression.zstd.compress(data, level=None, options=None, zstd_dict=None)
   ...
   Compress _data_ (a _bytes-like object_), returning the compressed
   data as a "bytes" object.

   The _level_ argument is an integer controlling the level of
   compression. _level_ is an alternative to setting
   "CompressionParameter.compression_level" in _options_. Use
   "bounds()" on "compression_level" to get the values that can be
   passed for _level_. If advanced compression options are needed, the
   _level_ argument must be omitted and in the _options_ dictionary
   the "CompressionParameter.compression_level" parameter should be
   set.

   The _options_ argument is a Python dictionary containing advanced
   ...
   The _zstd_dict_ argument is an instance of "ZstdDict" containing
...
compression.zstd.decompress(data, zstd_dict=None, options=None)
   ...
   Decompress _data_ (a _bytes-like object_), returning the
   ...
   If _data_ is the concatenation of multiple distinct compressed
...
class compression.zstd.ZstdCompressor(level=None, options=None, zstd_dict=None)
   ...
   The _zstd_dict_ argument is an optional instance of "ZstdDict"
   ...
   compress(data, mode=ZstdCompressor.CONTINUE)
      ...
      "compress()" is called with _mode_ set to "FLUSH_FRAME",
   ...
   set_pledged_input_size(size)
      ...
      Specify the amount of uncompressed data _size_ that will be
      provided for the next frame. _size_ will be written into the
      frame header of the next frame unless
      "CompressionParameter.content_size_flag" is "False" or "0". A
      size of "0" means that the frame is empty. If _size_ is "None",
   ...
   FLUSH_FRAME
      ...
      "compress()" will be written into a new frame and _cannot_
      reference past data.
...
class compression.zstd.ZstdDecompressor(zstd_dict=None, options=None)
   ...
   Note:
     ...
     should use "decompress()", "ZstdFile" if working with a _file
     object_, or multiple "ZstdDecompressor" instances.
   ...
   decompress(data, max_length=-1)
      ...
      If _max_length_ is non-negative, the method returns at most
      _max_length_ bytes of decompressed data. If this limit is
      reached and further output can be produced, the "needs_input"
      attribute will be set to "False". In this case, the next call to
      "decompress()" may provide _data_ as "b''" to obtain more of the
      output.
...
compression.zstd.train_dict(samples, dict_size)
   ...
   The _samples_ argument (an iterable of "bytes" objects), is the
   population of samples used to train the Zstandard dictionary.

   The _dict_size_ argument, an integer, is the maximum size (in
...
compression.zstd.finalize_dict(zstd_dict, /, samples, dict_size, level)
   ...
   The _zstd_dict_ argument is a "ZstdDict" instance with the
   "dict_content" containing the raw dictionary contents.

   The _samples_ argument (an iterable of "bytes" objects), contains
   ...
   The _level_ argument (an integer) is the compression level expected
...
class compression.zstd.ZstdDict(dict_content, /, *, is_raw=False)
   ...
   The _dict_content_ argument (a _bytes-like object_), is the already
   trained dictionary information.

   The _is_raw_ argument, a boolean, is an advanced parameter
   controlling the meaning of _dict_content_. "True" means
   _dict_content_ is a “raw content” dictionary, without any format
   restrictions. "False" means _dict_content_ is an ordinary Zstandard
   ...
   overhead of loading the dictionary.
      ...
      | be         | target_length" |            |
      | overridden | , "strategy",  |            |
      | by the dic | "enable_long_d |            |
      | tionary’s  | istance_matchi |            |
      | parameters | ng", "ldm_hash |            |
      |            | _log", "ldm_mi |            |
      |            | n_match", "ldm |            |
      |            | _bucket_size_l |            |
      |            | og", "ldm_hash |            |
   ...
   decompressing.
      ...
      dict_content
         ...
         It’s the same as the _dict_content_ argument in the
