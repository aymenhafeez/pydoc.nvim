...
If the system does not support the C-level _backtrace(3)_ or
_dladdr1(3)_, then C stack dumps will not work. An error will be
printed instead of the stack.

Additionally, some compilers do not support _CPython’s_ implementation
...
faulthandler.enable(file=sys.stderr, all_threads=True, c_stack=True)
   ...
   If _c_stack_ is "True", then the C stack trace is printed after the
   ...
   _GIL_ is disabled to prevent the risk of data races.
