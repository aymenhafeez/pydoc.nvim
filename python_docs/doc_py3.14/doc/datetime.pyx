Python 3.14.2
*datetime.pyx*                                Last change: 2025 Dec 20
...
See also:
  ...
  Package DateType
     ...
     allow _static type checkers_ to differentiate between naive and
     aware datetimes.
...
classmethod date.strptime(date_string, format)
   ...
   Return a "date" corresponding to _date_string_, parsed according to
   _format_. This is equivalent to:
>
...
<
   ...
   Note:
     ...
     If _format_ specifies a day of month without a year a
     ...
     leap year. Such _format_ values may raise an error as of Python
     3.15.  The workaround is to always include a year in your
     _format_.  If parsing _date_string_ values that do not have a
...
5. _date1_ is considered less than _date2_ when _date1_ precedes
   _date2_ in time. In other words, "date1 < date2" if and only if
...
classmethod datetime.strptime(date_string, format)
   ...
   If _format_ does not contain microseconds or time zone information,
...
<
   ...
   Changed in version 3.13: If _format_ specifies a day of month
   ...
   format is not a leap year. Such _format_ values may raise an error
   as of Python 3.15.  The workaround is to always include a year in
   your _format_.  If parsing _date_string_ values that do not have a
   year, explicitly add a year that is a leap year before parsing:
...
5. _datetime1_ is considered less than _datetime2_ when _datetime1_
   precedes _datetime2_ in time, taking into account the time zone.
...
class datetime.time(hour=0, minute=0, second=0, microsecond=0, tzinfo=None, *, fold=0)
   ...
   * "fold in [0, 1]".
   ...
   If an argument outside those ranges is given, "ValueError" is
   raised. All default to 0 except _tzinfo_, which defaults to "None".
...
classmethod time.strptime(date_string, format)
   ...
   Return a "time" corresponding to _date_string_, parsed according to
   _format_.
...
<
   "ValueError" is raised if the _date_string_ and _format_ cannot be
...
tzinfo.tzname(dt)
   ...
   on the specific value of _dt_ passed, especially if the "tzinfo"
...
See also:
     ...
     "zoneinfo"
        ...
        "zoneinfo" brings the _IANA time zone database_ (also known as
