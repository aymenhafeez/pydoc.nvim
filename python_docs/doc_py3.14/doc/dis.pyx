...
class dis.Bytecode(x, *, first_line=None, current_offset=None, show_caches=False, adaptive=False, show_offsets=False, show_positions=False)
   ...
   If _show_offsets_ is "True", "dis()" will include instruction
   offsets in the output.

   If _show_positions_ is "True", "dis()" will include instruction
...
dis.disco(code, lasti=-1, *, file=None, show_caches=False, adaptive=False, show_offsets=False, show_positions=False)
   ...
   1. the source code location of the instruction. Complete location
      information is shown if _show_positions_ is true. Otherwise (the
...
dis.get_instructions(x, *, first_line=None, show_caches=False, adaptive=False)
   ...
   The _adaptive_ parameter works as it does in "dis()".
   ...
   Changed in version 3.13: The _show_caches_ parameter is deprecated
   and has no effect. The iterator generates the "Instruction"
   instances with the _cache_info_ field populated (regardless of the
   value of _show_caches_) and it no longer generates separate items
...
LOAD_FROM_DICT_OR_DEREF(i)
   ...
   similar to "LOAD_DEREF". This is used for loading _closure
   variables_ in class bodies (which previously used
...
COPY_FREE_VARS(n)
   ...
   Copies the "n" _free (closure) variables_ from the closure into the
...
SET_FUNCTION_ATTRIBUTE(flag)
   ...
   * "0x10" the _annotate function_ for the function object
...
dis.hasfree
   ...
   Sequence of bytecodes that access a _free (closure) variable_.
