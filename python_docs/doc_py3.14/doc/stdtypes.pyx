...
The "complex" type implements the "numbers.Complex" _abstract base
class_. "complex" also has the following additional methods.
...
|                            | itself _n_ times                 |            |
+----------------------------+----------------------------------+------------+
| "s[i]"                     | _i_th item of _s_, origin 0      | (3)(8)     |
+----------------------------+----------------------------------+------------+
...
8. An "IndexError" is raised if _i_ is outside the sequence range.
...
sequence.count(value, /)
   ...
   Return the total number of occurrences of _value_ in _sequence_.

sequence.index(value[, start[, stop])

   Return the index of the first occurrence of _value_ in _sequence_.

   Raises "ValueError" if _value_ is not found in _sequence_.

   The _start_ or _stop_ arguments allow for efficient searching of
   subsections of the sequence, beginning at _start_ and ending at
   _stop_. This is roughly equivalent to "start +
   sequence[start:stop].index(value)", only without copying any data.

   Caution:

     Not all sequence types support passing the _start_ and _stop_
     arguments.
...
+--------------------------------+----------------------------------+-----------------------+
| "s += t"                       | extends _s_ with the contents of |                       |
|                                | _t_ (for the most part the same  |                       |
|                                | as "s[len(s):len(s)] = t")       |                       |
+--------------------------------+----------------------------------+-----------------------+
| "s *= n"                       | updates _s_ with its contents    | (2)                   |
|                                | repeated _n_ times               |                       |
+--------------------------------+----------------------------------+-----------------------+
...
2. The value _n_ is an integer, or an object implementing
...
sequence.append(value, /)
   ...
   Append _value_ to the end of the sequence This is equivalent to
...
sequence.clear()
   ...
   Remove all items from _sequence_. This is equivalent to writing
   "del sequence[:]".
...
sequence.copy()
   ...
   Create a shallow copy of _sequence_. This is equivalent to writing
   "sequence[:]".
...
sequence.extend(iterable, /)
   ...
   Extend _sequence_ with the contents of _iterable_. For the most
   part, this is the same as writing "seq[len(seq):len(seq)] =
   iterable".

sequence.insert(index, value, /)

   Insert _value_ into _sequence_ at the given _index_. This is
   equivalent to writing "sequence[index:index] = [value]".

sequence.pop(index=-1, /)

   Retrieve the item at _index_ and also removes it from _sequence_.
   By default, the last item in _sequence_ is removed and returned.

sequence.remove(value, /)

   Remove the first item from _sequence_ where "sequence[i] == value".

   Raises "ValueError" if _value_ is not found in _sequence_.

sequence.reverse()

   Reverse the items of _sequence_ in place. This method maintains
...
str.endswith(suffix[, start[, end]])
   ...
   Using _start_ and _end_ is equivalent to
...
str.find(sub[, start[, end]])
   ...
   _sub_ is not found. For example:
>
...
static str.maketrans(from, to, remove='', /)
   ...
   and in the resulting dictionary, each character in _from_ will be
   mapped to the character at the same position in _to_.  If there is
...
str.replace(old, new, /, count=-1)
   ...
   replaced by _new_.  If _count_ is given, only the first _count_
   occurrences are replaced. If _count_ is not specified or "-1", then
   all occurrences are replaced.

   Changed in version 3.13: _count_ is now supported as a keyword
   argument.
...
>
      ...
      ['1', '2', '3']
<
   If _sep_ is not specified or is "None" and  _maxsplit_ is "0", only
...
An _f-string_ (formally a _formatted string literal_) is a string
...
_replacement fields_, which are delimited by curly brackets ("{}").
Each replacement field must contain an expression, optionally followed
by:

* a _debug specifier_ – an equal sign ("=");

* a _conversion specifier_ – "!s", "!r" or "!a"; and/or

* a _format specifier_ prefixed with a colon (":").
...
function. If the replacement field includes a _format specifier_
...
An _t-string_ (formally a _template string literal_) is a string
...
class bytes(source, encoding, errors='strict')
   ...
   * Copying existing binary data via the buffer protocol:
   ...
   classmethod fromhex(string, /)
      ...
      "bytes" and _bytes-like objects_ as input.
...
class bytearray(source, encoding, errors='strict')
   ...
   * Copying existing binary data via the buffer protocol:
   ...
   resize(size, /)
      ...
      Resize the "bytearray" to contain _size_ bytes. _size_ must be
      greater than or equal to 0.

      If the "bytearray" needs to shrink, bytes beyond _size_ are
      truncated.

      If the "bytearray" needs to grow, all new bytes, those beyond
      _size_, will be set to null bytes.
...
bytearray.lstrip(bytes=None, /)
   ...
   The _bytes_ argument is a binary sequence specifying the set of
   byte values to be removed.  If omitted or "None", the _bytes_
   argument defaults to removing ASCII whitespace.  The _bytes_
...
bytearray.rstrip(bytes=None, /)
   ...
   removed.  The _bytes_ argument is a binary sequence specifying the
   set of byte values to be removed.  If omitted or "None", the
   _bytes_ argument defaults to removing ASCII whitespace.  The
   _bytes_ argument is not a suffix; rather, all combinations of its
   values are stripped:
>
      >>> b'   spacious   '.rstrip()
      b'   spacious'
...
bytearray.strip(bytes=None, /)
   ...
   bytes removed. The _bytes_ argument is a binary sequence specifying
   the set of byte values to be removed.  If omitted or "None", the
   _bytes_ argument defaults to removing ASCII whitespace. The _bytes_
...
class memoryview(object)
      ...
      A "memoryview" has the notion of an _element_, which is the
      atomic memory unit handled by the originating _object_.  For
...
<
      ...
      is also supported and returns a single _element_ with the
      correct type.  One-dimensional memoryviews can be indexed with
      an integer or a one-integer tuple.  Multi-dimensional
      memoryviews can be indexed with tuples of exactly _ndim_
      integers where _ndim_ is the number of dimensions.  Zero-
...
<
      ...
      Changed in version 3.5: memoryviews can now be indexed with
      tuple of integers.

      Changed in version 3.14: memoryview is now a _generic type_.
...
<
         ...
         _not_ imply "v == w" for memoryview objects.
...
<
         ...
         views, the data is converted to C first. _order=None_ is the
         same as _order=’C’_.
...
<
         ...
         _bytes_per_sep_ parameters to insert separators between bytes
         in the hex output.
...
<
      ...
      cast(format, shape, /)
         ...
         Cast a memoryview to a new format or shape. _shape_ defaults
         ...
         casts are 1D -> C-_contiguous_ and C-contiguous -> 1D.
...
<
      ...
      count(value, /)
         ...
         Count the number of occurrences of _value_.

         Added in version 3.14.

   index(value, start=0, stop=sys.maxsize, /)

         Return the index of the first occurrence of _value_ (at or
         after index _start_ and before index _stop_).

         Raises a "ValueError" if _value_ cannot be found.
...
set < other
   ...
   Test whether the set is a proper subset of _other_, that is, "set
   <= other and set != other".
...
set > other
   ...
   Test whether the set is a proper superset of _other_, that is, "set
   >= other and set != other".
...
set ^ other
   ...
   Return a new set with elements in either the set or _other_ but not
   both.
...
not equal and are not subsets of each other, so _all_ of the following
...
set.remove(elem, /)
   ...
   Remove element _elem_ from the set.  Raises "KeyError" if _elem_ is
...
<
   ...
   update(iterable, /, **kwargs)
      ...
      Update the dictionary with the key/value pairs from _mapping_ or
      _iterable_ and _kwargs_, overwriting existing keys.  Return
      "None".
...
* The default Python prompt of the _interactive_ shell when partial
  input is incomplete.
