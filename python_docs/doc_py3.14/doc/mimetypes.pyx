...
mimetypes.guess_type(url, strict=True)
   ...
   internal database can be identified. When _strict_ is "True" (the
   default), only the IANA types are supported; when _strict_ is
   "False", some additional non-standard but commonly used MIME types
   are also recognized.

   Changed in version 3.8: Added support for _url_ being a _path-like
   object_.

   Deprecated since version 3.13: Passing a file path instead of URL
   is _soft deprecated_. Use "guess_file_type()" for this.
...
mimetypes.guess_file_type(path, *, strict=True)
   ...
   of URL. Path can be a string, a bytes object or a _path-like
   object_.
...
mimetypes.guess_extension(type, strict=True)
   ...
   be mapped to the MIME type _type_ by "guess_type()" and
   "guess_file_type()". If no extension can be guessed for _type_,
   "None" is returned.
