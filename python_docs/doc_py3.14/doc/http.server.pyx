...
class http.server.HTTPSServer(server_address, RequestHandlerClass, bind_and_activate=True, *, certfile, keyfile=None, password=None, alpn_protocols=None)
   ...
   The _certfile_ argument is the path to the SSL certificate chain
   file, and the _keyfile_ is the path to file containing the private
   key.

   A _password_ can be specified for files protected and wrapped with
   ...
   See also:
     ...
     on the accepted values for _certfile_, _keyfile_ and _password_.

   When specified, the _alpn_protocols_ argument must be a sequence of
...
"ThreadingHTTPSServer" must be given a _RequestHandlerClass_ on
...
class http.server.BaseHTTPRequestHandler(request, client_address, server)
   ...
   responses
      ...
      "{code: (shortmessage, longmessage)}". The _shortmessage_ is
      usually used as the _message_ key in an error response, and
      _longmessage_ as the _explain_ key.  It is used by
   ...
   send_error(code, message=None, explain=None)
      ...
      _code_ specifies the HTTP error code, with _message_ as an
      optional, short, human readable description of the error.  The
      _explain_ argument can be used to provide more detailed
      ...
      holds the default values for _message_ and _explain_ that will
   ...
   send_header(keyword, value)
      ...
      "flush_headers()" is invoked. _keyword_ should specify the
      header keyword, with _value_ specifying its value. Note that,
   ...
   log_request(code='-', size='-')
      ...
      Logs an accepted (successful) request. _code_ should specify the
      numeric HTTP code associated with the response. If a size of the
      response is available, then it should be passed as the _size_
      parameter.
   ...
   log_error(...)
      ...
      arguments (_format_ and additional values).
...
class http.server.SimpleHTTPRequestHandler(request, client_address, server, directory=None)
   ...
   This class serves files from the directory _directory_ and below,
   or the current directory if _directory_ is not provided, directly
   mapping the directory structure to HTTP requests.

   Changed in version 3.7: Added the _directory_ parameter.

   Changed in version 3.9: The _directory_ parameter accepts a _path-
   like object_.
   ...
   do_GET()
      ...
      which in turn uses the _extensions_map_ variable, and the file
...
<
   This option requires _–tls-cert`_ to be specified.
