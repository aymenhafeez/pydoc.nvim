Python 3.14.2
*heapq.pyx*                                   Last change: 2025 Dec 20
...
heap[2*k+1]" and "heap[k] <= heap[2*k+2]" for all _k_ for which the
...
_greater_ than any of its children.  These are implemented as lists
for which "maxheap[2*k+1] <= maxheap[k]" and "maxheap[2*k+2] <=
maxheap[k]" for all _k_ for which the compared elements exist. The
root, "maxheap[0]", contains the _largest_ element;
...
_heap_ generally refers to a min-heap. The API for max-heaps is named
using a "_max"  suffix.
...
heapq.heappush(heap, item)
   ...
   Push the value _item_ onto the _heap_, maintaining the min-heap
   invariant.
...
heapq.heappush_max(heap, item)
   ...
   Push the value _item_ onto the max-heap _heap_, maintaining the
   max-heap invariant.
...
heapq.heappop_max(heap)
   ...
   Pop and return the largest item from the max-heap _heap_,
...
heapq.heappushpop_max(heap, item)
   ...
   Push _item_ on the max-heap _heap_, then pop and return the largest
   item from _heap_. The combined action runs more efficiently than
...
heapq.heapreplace_max(heap, item)
   ...
   Pop and return the largest item from the max-heap _heap_ and also
   push the new _item_. The max-heap size doesn’t change. If the max-
   heap is empty, "IndexError" is raised.

   The value returned may be smaller than the _item_ added.  Refer to
