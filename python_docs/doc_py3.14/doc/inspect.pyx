...
|                   |                     | created, or "None". See "sy |
|                   |                     | s.set_coroutine_origin_trac |
|                   |                     | king_depth()"               |
+-------------------+---------------------+-----------------------------+
...
inspect.ispackage(object)
   ...
   Return "True" if the object is a _package_.
...
inspect.iscoroutinefunction(object)
   ...
   Changed in version 3.13: Functions wrapped in
   "functools.partialmethod()" now return "True" if the wrapped
   function is a _coroutine function_.

inspect.markcoroutinefunction(func)

   Decorator to mark a callable as a _coroutine function_ if it would
   not otherwise be detected by "iscoroutinefunction()".

   This may be of use for sync functions that return a _coroutine_, if
...
inspect.signature(callable, *, follow_wrapped=True, globals=None, locals=None, eval_str=False, annotation_format=Format.VALUE)
   ...
   Return a "Signature" object for the given _callable_:
>
...
<
   ...
   "annotationlib.get_annotations()".  The _globals_, _locals_, and
   _eval_str_ parameters are passed into
   ...
   _annotation_format_ parameter to control the format of the returned
   ...
   annotations are stringized, and _eval_str_ is not false, the
   ...
   Changed in version 3.5: The _follow_wrapped_ parameter was added.
   Pass "False" to get a signature of _callable_ specifically
   ...
   Changed in version 3.14: The _annotation_format_ parameter was
   added.
...
class inspect.Signature(parameters=None, *, return_annotation=Signature.empty)
   ...
   "Signature" objects are _immutable_.  Use "Signature.replace()" or
   ...
   replace(*[, parameters][, return_annotation])
      ...
      _parameters_ and/or _return_annotation_ to override the
...
<
   ...
   format(*, max_width=None, quote_annotation_strings=True)
      ...
      If _max_width_ is passed, the method will attempt to fit the
      signature into lines of at most _max_width_ characters. If the
      signature is longer than _max_width_, all parameters will be on
      separate lines.

      If _quote_annotation_strings_ is False, _annotations_ in the
      ...
      Changed in version 3.14: The _unquote_annotations_ parameter was
      added.
...
class inspect.FrameInfo
   ...
   Changed in version 3.5: Return a _named tuple_ instead of a
   "tuple".

   Changed in version 3.11: "FrameInfo" is now a class instance (that
   is backwards compatible with the previous _named tuple_).
...
inspect.getinnerframes(traceback, context=1)
   ...
   of _frame_. The first entry in the list represents _traceback_; the
