...
locale.setlocale(category, locale=None)
   ...
   If _locale_ is given and not "None", "setlocale()" modifies the
   locale setting for the _category_. The available categories are
   listed in the data description below. _locale_ may be a string, or
   ...
   If _locale_ is a pair, it is converted to a locale name using the
...
locale.getlocale(category=LC_CTYPE)
   ...
   tuple containing the language code and encoding. _category_ may be
...
locale.getpreferredencoding(do_setlocale=True)
   ...
   Return the _locale encoding_ used for text data, according to user
   ...
   invoking setlocale is not necessary or desired, _do_setlocale_
   should be set to "False".

   On Android or if the Python UTF-8 Mode is enabled, always return
   "'utf-8'", the _locale encoding_ and the _do_setlocale_ argument
   are ignored.

   The Python preinitialization configures the LC_CTYPE locale. See
   also the _filesystem encoding and error handler_.
...
locale.getencoding()
   ...
   Get the current _locale encoding_:
...
locale.strcoll(string1, string2)
   ...
   value, or "0", depending on whether _string1_ collates before or
   after _string2_ or is equal to it.
...
locale.currency(val, symbol=True, grouping=False, international=False)
   ...
   Formats a number _val_ according to the current "LC_MONETARY"
   settings.
...
locale.atof(string, func=float)
   ...
   by calling _func_ on the result of calling "delocalize()" on
   _string_.
...
where _language_ is a two- or three-letter language code from ISO 639,
_territory_ is a two-letter country or region code from ISO 3166,
_charset_ is a locale encoding, and _modifier_ is a script name, a
...
where _language_ and _territory_ have the same meaning as in Posix,
_script_ is a four-letter script code from ISO 15924, and _modifier_
...
where _language_ and _territory_ are full names, such as “English” and
“United States”, and _charset_ is either a code page number (for
