...
class mailbox.Mailbox
   ...
   The default "Mailbox" _iterator_ iterates over message
   ...
   iterkeys()
      ...
      Return an _iterator_ over all keys

   keys()

      The same as "iterkeys()", except that a "list" is returned
      rather than an _iterator_

   itervalues()
   __iter__()

      Return an _iterator_ over representations of all messages. The
   ...
   iteritems()
      ...
      Return an _iterator_ over (_key_, _message_) pairs, where _key_
      is a key and _message_ is a message representation. The messages
   ...
   items()
      ...
      returned rather than an _iterator_ of pairs.
   ...
   get_file(key)
      ...
      Return a _file-like_ representation of the message corresponding
      ...
      Changed in version 3.2: The file object really is a _binary
      file_; previously it was incorrectly returned in text mode.
      Also, the _file-like object_ now supports the _context manager_
      protocol: you can use a "with" statement to automatically close
      it.

      Note:

        Unlike other representations of messages, _file-like_
...
<
   ...
   get_flags(key)
      ...
      corresponding to _key_. This is the same as
   ...
   set_flags(key, flags)
      ...
      On the message corresponding to _key_, set the flags specified
      by _flags_ and unset all others. Calling
...
<
   ...
   add_flag(key, flag)
      ...
      On the message corresponding to _key_, set the flags specified
      by _flag_ without changing other flags. To add more than one
      flag at a time, _flag_ may be a string of more than one
      character.
   ...
   remove_flag(key, flag)
      ...
      On the message corresponding to _key_, unset the flags specified
      by _flag_ without changing other flags. To remove more than one
      flag at a time, _flag_ may be a string of more than one
      character.
   ...
   set_info(key, info)
      ...
      Set the info of the message corresponding to _key_ to _info_.
