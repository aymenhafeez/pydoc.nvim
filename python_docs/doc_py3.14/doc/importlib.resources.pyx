...
* _anchor_ is an "Anchor", as in "files()". Unlike in "files", it may
  not be omitted.

* _path_names_ are components of a resource’s path name, relative to
...
<
  ...
  an explicit _encoding_ argument if multiple _path_names_ are given.
...
importlib.resources.open_binary(anchor, *path_names)
   ...
   See the introduction for details on _anchor_ and _path_names_.
...
importlib.resources.open_text(anchor, *path_names, encoding='utf-8', errors='strict')
   ...
   See the introduction for details on _anchor_ and _path_names_.
   _encoding_ and _errors_ have the same meaning as in built-in
   "open()".

   For backward compatibility reasons, the _encoding_ argument must be
   given explicitly if there are multiple _path_names_. This
...
importlib.resources.path(anchor, *path_names)
   ...
   Provides the path to the _resource_ as an actual file system path.
