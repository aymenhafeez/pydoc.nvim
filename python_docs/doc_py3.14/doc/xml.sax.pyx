...
xml.sax.make_parser(parser_list=[])
   ...
   will be used.  If _parser_list_ is provided, it must be an iterable
   ...
   Changed in version 3.8: The _parser_list_ argument can be any
...
xml.sax.parse(filename_or_stream, handler, error_handler=handler.ErrorHandler())
   ...
   object.  The _handler_ parameter needs to be a SAX "ContentHandler"
   instance.  If _error_handler_ is given, it must be a SAX
   "ErrorHandler" instance; if omitted,  "SAXParseException" will be
   raised on all errors.  There is no return value; all work must be
   done by the _handler_ passed in.

xml.sax.parseString(string, handler, error_handler=handler.ErrorHandler())

   Similar to "parse()", but parses from a buffer _string_ received as
   a parameter.  _string_ must be a "str" instance or a _bytes-like
   object_.
