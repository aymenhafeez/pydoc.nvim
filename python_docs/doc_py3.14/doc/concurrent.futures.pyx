...
<
   map(fn, *iterables, timeout=None, chunksize=1, buffersize=None)
      ...
      * The _iterables_ are collected immediately rather than lazily,
        unless a _buffersize_ is specified to limit the number of
        submitted tasks whose results have not yet been yielded. If
        the buffer is full, iteration over the _iterables_ pauses
        until a result is yielded from the buffer.

      * _fn_ is executed asynchronously and several calls to _fn_ may
        be made concurrently.

      The returned iterator raises a "TimeoutError" if "__next__()" is
      called and the result isn’t available after _timeout_ seconds
      from the original call to "Executor.map()". _timeout_ can be an
      int or a float.  If _timeout_ is not specified or "None", there
      is no limit to the wait time.

      If a _fn_ call raises an exception, then that exception will be
      raised when its value is retrieved from the iterator.

      When using "ProcessPoolExecutor", this method chops _iterables_
      into a number of chunks which it submits to the pool as separate
      tasks.  The (approximate) size of these chunks can be specified
      by setting _chunksize_ to a positive integer.  For very long
      iterables, using a large value for _chunksize_ can significantly
      improve performance compared to the default size of 1.  With
      "ThreadPoolExecutor" and "InterpreterPoolExecutor", _chunksize_
      has no effect.
   ...
   shutdown(wait=True, *, cancel_futures=False)
      ...
      If _wait_ is "True" then this method will not return until all
      ...
      _wait_, the entire Python program will not exit until all
      ...
      If both _cancel_futures_ and _wait_ are "True", all futures that
      ...
      the executor as a _context manager_ via the  "with" statement,
      which will shutdown the "Executor" (waiting as if
      "Executor.shutdown()" were called with _wait_ set to "True"):
>
         import shutil
...
class concurrent.futures.ThreadPoolExecutor(max_workers=None, thread_name_prefix='', initializer=None, initargs=())
   ...
   this is executed _before_ any exit handlers added using "atexit".
   ...
   Changed in version 3.6: Added the _thread_name_prefix_ parameter to
   ...
   Changed in version 3.13: Default value of _max_workers_ is changed
...
true multi-core parallelism.  Each interpreter has its own _Global
Interpreter Lock_, so code running in one interpreter can run on one
...
class concurrent.futures.InterpreterPoolExecutor(max_workers=None, thread_name_prefix='', initializer=None, initargs=())
   ...
   using a pool of at most _max_workers_ threads.  Each thread runs
   ...
   interpreter has its own _Global Interpreter Lock_, which means code
   run with this executor has true multi-core parallelism.

   The optional _initializer_ and _initargs_ arguments have the same
   meaning as for "ThreadPoolExecutor": the initializer is run when
   each worker is created, though in this case it is run in the
   worker’s interpreter.  The executor serializes the _initializer_
   and _initargs_ using "pickle" when sending them to the worker’s
   interpreter.

   Note:

     The executor may replace uncaught exceptions from _initializer_
...
class concurrent.futures.ProcessPoolExecutor(max_workers=None, mp_context=None, initializer=None, initargs=(), max_tasks_per_child=None)
   ...
   If _max_workers_ is less than or equal to "0", then a "ValueError"
   will be raised. On Windows, _max_workers_ must be less than or
   equal to "61". If it is not then "ValueError" will be raised. If
   _max_workers_ is "None", then the default chosen will be at most
   "61", even if more processors are available. _mp_context_ can be a
   "multiprocessing" context or "None". It will be used to launch the
   workers. If _mp_context_ is "None" or not given, the default
   ...
   _max_tasks_per_child_ is an optional argument that specifies the
   ...
   _max_tasks_per_child_ is "None" which means worker processes will
   ...
   a _mp_context_ parameter. This feature is incompatible with the
   “fork” start method.
   ...
   Changed in version 3.11: The _max_tasks_per_child_ argument was
   ...
   _mp_context_ configured to use a different start method. See the
   "os.fork()" documentation for further explanation.

   Changed in version 3.13: _max_workers_ uses
   "os.process_cpu_count()" by default, instead of "os.cpu_count()".

   Changed in version 3.14: The default process start method (see
   Contexts and start methods) changed away from _fork_. If you
   require the _fork_ start method for "ProcessPoolExecutor" you must
...
class concurrent.futures.Future
   ...
   result(timeout=None)
      ...
      "TimeoutError" will be raised. _timeout_ can be an int or float.
      If _timeout_ is not specified or "None", there is no limit to
      the wait time.
   ...
   exception(timeout=None)
      ...
      completed then this method will wait up to _timeout_ seconds.
      If the call hasn’t completed in _timeout_ seconds, then a
      "TimeoutError" will be raised.  _timeout_ can be an int or
      float.  If _timeout_ is not specified or "None", there is no
   ...
   add_done_callback(fn)
      ...
      Attaches the callable _fn_ to the future.  _fn_ will be called,
      ...
      If the future has already completed or been cancelled, _fn_ will
...
concurrent.futures.as_completed(fs, timeout=None)
   ...
   _timeout_ seconds from the original call to "as_completed()".
   _timeout_ can be an int or float. If _timeout_ is not specified or
