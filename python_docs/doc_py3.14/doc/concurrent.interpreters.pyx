Python 3.14.2
*concurrent.interpreters.pyx*                 Last change: 2025 Dec 20
...
runtime will use _in the current thread_.  That isolation makes them
...
thread, has its own _current_ execution context.  Multiple threads can
...
they do not share the _GIL_, which means combining threads with
...
provided by the _GIL_.  There are various options for dealing with
...
concurrent.interpreters.get_main()
   ...
   the interpreter the runtime created to run the _REPL_ or the script
