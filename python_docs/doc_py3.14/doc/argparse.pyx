...
class argparse.ArgumentParser(prog=None, usage=None, description=None, epilog=None, parents=[], formatter_class=argparse.HelpFormatter, prefix_chars='-', fromfile_prefix_chars=None, argument_default=None, conflict_handler='error', add_help=True, allow_abbrev=True, exit_on_error=True, *, suggest_on_error=False, color=True)
   ...
   * color - Allow color output (default: "True")
   ...
   Changed in version 3.8: In previous versions, _allow_abbrev_ also
   disabled grouping of short flags such as "-vv" to mean "-v -v".

   Changed in version 3.9: _exit_on_error_ parameter was added.

   Changed in version 3.14: _suggest_on_error_ and _color_ parameters
   were added.
...
<
Changed in version 3.14: The default "prog" value now reflects how
...
"ArgumentParser" uses _filesystem encoding and error handler_ to read
...
_filesystem encoding and error handler_. Arguments file should be
...
method of an "ArgumentParser", it will print a _message_ to
...
_default_ will not overwrite it:
>
   >>> parser = argparse.ArgumentParser()
   >>> parser.add_argument('--foo', default=42)
...
_choices_ keyword argument to "add_argument()".  When the command line
...
>
   ...
   'paper', 'scissors')
<
Any sequence can be passed as the _choices_ value, so "list" objects,
...
Note that _choices_ are checked after any type conversions have been
performed, so objects in _choices_ should match the type specified.
This can make _choices_ appear unfamiliar in usage, help, or error
messages.

To keep _choices_ user-friendly, consider a custom type wrapper that
converts and formats values, or omit type and handle conversion in
your application code.

Formatted choices override the default _metavar_ which is normally
derived from _dest_.  This is usually what you want because the user
never sees the _dest_ parameter.  If this display isn’t desirable
...
bar" indicate _optional_ arguments, which can always be omitted at the
command line. To make an option _required_, "True" can be specified
...
Note:
  ...
  expect _options_ to be _optional_, and thus they should be avoided
  when possible.
...
class argparse.Action(option_strings, dest, nargs=None, const=None, default=None, type=None, choices=None, required=False, help=None, metavar=None)
   ...
   __call__(parser, namespace, values, option_string=None)
      ...
      * _parser_ - The "ArgumentParser" object which contains this
        action.

      * _namespace_ - The "Namespace" object that will be returned by
        "parse_args()".  Most actions add an attribute to this object
        using "setattr()".

      * _values_ - The associated command-line arguments, with any
        type conversions applied.  Type conversions are specified with
        the type keyword argument to "add_argument()".

      * _option_string_ - The option string that was used to invoke
...
ArgumentParser.add_subparsers(*[, title][, description][, prog][, parser_class][, action][, dest][, required][, help][, metavar])
   ...
   * _title_ - title for the sub-parser group in help output; by
     default “subcommands” if description is provided, otherwise uses
     title for positional arguments

   * _description_ - description for the sub-parser group in help
     output, by default "None"

   * _prog_ - usage information that will be displayed with sub-
     command help, by default the name of the program and any
     positional arguments before the subparser argument

   * _parser_class_ - class which will be used to create sub-parser
...
<
   Furthermore, "add_parser()" supports an additional _aliases_
...
>
      ...
      Namespace(foo='bar')
<
   "add_parser()" supports also an additional _deprecated_ argument,
...
<
   ...
   Changed in version 3.14: Subparser’s _prog_ is no longer affected
   by a custom usage message in the main parser.
...
<
   Note:
     ...
     If one argument uses _FileType_ and then a subsequent argument
   ...
   Changed in version 3.4: Added the _encodings_ and _errors_
   parameters.
...
<
   ...
   "add_argument_group()" method accepts _title_ and _description_
...
<
   ...
   Deprecated since version 3.14: Passing prefix_chars to
...
<
   ...
   _required_ argument, to indicate that at least one of the mutually
...
<
   ...
   support the _title_ and _description_ arguments of
...
ArgumentParser.print_usage(file=None)
   ...
   invoked on the command line.  If _file_ is "None", "sys.stdout" is
   assumed.
...
ArgumentParser.print_help(file=None)
   ...
   _file_ is "None", "sys.stdout" is assumed.
...
ArgumentParser.convert_arg_line_to_args(arg_line)
   ...
   _fromfile_prefix_chars_ keyword argument to the "ArgumentParser"
...
ArgumentParser.exit(status=0, message=None)
   ...
   _status_ and, if given, it prints a _message_ to "sys.stderr"
...
ArgumentParser.error(message)
   ...
   This method prints a usage message, including the _message_, to
...
ArgumentParser.register(registry_name, value, object)
   ...
   _registry_name_, specifying the internal registry where the object
   will be stored (e.g., "action", "type"), _value_, which is the key
