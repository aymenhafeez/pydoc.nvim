...
PurePath.joinpath(*pathsegments)
   ...
   of the given _pathsegments_ in turn:
>
...
>
      ...
      True
<
   Set _case_sensitive_ to "True" or "False" to override this
   behaviour.
...
>
      ...
      False
<
   Changed in version 3.12: The _pattern_ parameter accepts a _path-
   like object_.
...
PurePath.with_segments(*pathsegments)
   ...
   _pathsegments_. This method is called whenever a derivative path is
...
<
   ...
   raised no matter the value of _strict_.
...
<
   ...
   By default, or when the _recurse_symlinks_ keyword-only argument is
   set to "False", this method follows symlinks except when expanding
   “"**"” wildcards. Set _recurse_symlinks_ to "True" to always follow
   symlinks.
   ...
   Changed in version 3.13: The _recurse_symlinks_ parameter was
   added.

   Changed in version 3.13: The _pattern_ parameter accepts a _path-
   like object_.
...
Path.walk(top_down=True, on_error=None, follow_symlinks=False)
   ...
   For each directory in the directory tree rooted at _self_
   (including _self_ but excluding ‘.’ and ‘..’), the method yields a
   3-tuple of "(dirpath, dirnames, filenames)".

   _dirpath_ is a "Path" to the directory currently being walked,
   _dirnames_ is a list of strings for the names of subdirectories in
   _dirpath_ (excluding "'.'" and "'..'"), and _filenames_ is a list
   of strings for the names of the non-directory files in _dirpath_.
   To get a full path (which begins with _self_) to a file or
   directory in _dirpath_, do "dirpath / name". Whether or not the
   lists are sorted is file system-dependent.

   If the optional argument _top_down_ is true (which is the default),
   ...
   _top_down_ is false, the triple for a directory is generated after
   the triples for all of its subdirectories (directories are walked
   bottom-up). No matter the value of _top_down_, the list of
   subdirectories is retrieved before the triples for the directory
   and its subdirectories are walked.

   When _top_down_ is true, the caller can modify the _dirnames_ list
   in-place (for example, using "del" or slice assignment), and
   "Path.walk()" will only recurse into the subdirectories whose names
   remain in _dirnames_. This can be used to prune the search, or to
   impose a specific order of visiting, or even to inform
   "Path.walk()" about directories the caller creates or renames
   before it resumes "Path.walk()" again. Modifying _dirnames_ when
   _top_down_ is false has no effect on the behavior of "Path.walk()"
   since the directories in _dirnames_ have already been generated by
   the time _dirnames_ is yielded to the caller.

   By default, errors from "os.scandir()" are ignored.  If the
   optional argument _on_error_ is specified, it should be a callable;
   ...
   instead adds them to the _filenames_ list. Set _follow_symlinks_ to
   true to resolve symlinks and place them in _dirnames_ and
   _filenames_ as appropriate for their targets, and consequently
   visit directories pointed to by symlinks (where supported).

   Note:

     Be aware that setting _follow_symlinks_ to true can lead to
   ...
   Note:
     ...
     during execution. For example, if a directory from _dirnames_ has
     been replaced with a symlink and _follow_symlinks_ is false,
     "Path.walk()" will still try to descend into it. To prevent such
     behavior, remove directories from _dirnames_ as appropriate.

   Note:

     Unlike "os.walk()", "Path.walk()" lists symlinks to directories
     in _filenames_ if _follow_symlinks_ is false.
...
Path.mkdir(mode=0o777, parents=False, exist_ok=False)
   ...
   If _exist_ok_ is true, "FileExistsError" will not be raised unless
...
Path.copy(target, *, follow_symlinks=True, preserve_metadata=False)
   ...
   Copy this file or directory tree to the given _target_, and return
   a new "Path" instance pointing to _target_.

   If the source is a file, the target will be replaced if it is an
   existing file. If the source is a symlink and _follow_symlinks_ is
   true (the default), the symlink’s target is copied. Otherwise, the
   symlink is recreated at the destination.

   If _preserve_metadata_ is false (the default), only directory
   structures and file data are guaranteed to be copied. Set
   _preserve_metadata_ to true to ensure that file and directory
...
Path.copy_into(target_dir, *, follow_symlinks=True, preserve_metadata=False)
   ...
   Copy this file or directory tree into the given _target_dir_, which
...
Path.move(target)
   ...
   Move this file or directory tree to the given _target_, and return
   a new "Path" instance pointing to _target_.

   If the _target_ doesn’t exist it will be created. If both this path
   and the _target_ are existing files, then the target is
   overwritten. If both paths point to the same file or directory, or
   the _target_ is a non-empty directory, then "OSError" is raised.
...
Path.move_into(target_dir)
   ...
   Move this file or directory tree into the given _target_dir_, which
...
"[seq]"
   Matches one character in _seq_, where _seq_ is a sequence of
...
"[!seq]"
   Matches one character not in _seq_, where _seq_ follows the same
   rules as above.
...
5. The values returned from pathlib’s "path.glob()" and "path.rglob()"
   include the _path_ as a prefix, unlike the results of
   "glob.glob(root_dir=path)".

6. The values returned from pathlib’s "path.glob()" and "path.rglob()"
   may include _path_ itself, for example when globbing “"**"”,
   whereas the results of "glob.glob(root_dir=path)" never include an
   empty string that would correspond to _path_.
...
objects, and so it’s said to be _object-oriented_. On the other hand,
the "os" and "os.path" modules supply functions that work with low-
level "str" and "bytes" objects, which is a more _procedural_
...
class pathlib.types.PathInfo
   ...
   exists(*, follow_symlinks=True)
      ...
      If _follow_symlinks_ is "False", return "True" for symlinks
   ...
   is_dir(*, follow_symlinks=True)
      ...
      If _follow_symlinks_ is "False", return "True" only if the path
