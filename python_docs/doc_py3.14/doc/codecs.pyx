...
codecs.charmap_build(string)
   ...
   encoding. Given a "str" _string_ of up to 256 characters
...
codecs.open(filename, mode='r', encoding=None, errors='strict', buffering=-1)
   ...
   Note:
     ...
     If _encoding_ is not "None", then the underlying encoded files
     are always opened in binary mode. No automatic conversion of
     "'\n'" is done on reading and writing. The _mode_ argument may be
...
codecs.iterencode(iterator, encoding, errors='strict', **kwargs)
   ...
   provided by _iterator_. _iterator_ must yield "str" objects. This
   function is a _generator_. The _errors_ argument (as well as any
...
codecs.iterdecode(iterator, encoding, errors='strict', **kwargs)
   ...
   provided by _iterator_. _iterator_ must yield "bytes" objects. This
   function is a _generator_. The _errors_ argument (as well as any
   other keyword argument) is passed through to the incremental
   decoder.
...
codecs.readbuffer_encode(buffer, errors=None, /)
   ...
   Return a "tuple" containing the raw bytes of _buffer_, a buffer-
...
|                           | point with formats "\x_hh_" "\u_xxxx_"          |
|                           | "\U_xxxxxxxx_". On decoding, use hexadecimal    |
|                           | form of byte value with format "\x_hh_".        |
...
|                           | code point with format "&#_num_;". Implemented  |
...
codecs.backslashreplace_errors(exception)
   ...
   formats "\x_hh_" "\u_xxxx_" "\U_xxxxxxxx_". On decoding, use the
   hexadecimal form of byte value with format "\x_hh_".
...
class codecs.Codec
   ...
   encode(input, errors='strict')
      ...
      length consumed). For instance, _text encoding_ converts a
   ...
   decode(input, errors='strict')
      ...
      interface – for example, buffer objects and memory mapped files.
...
codecs.escape_encode(input, errors=None)
   ...
   Encode _input_ using escape sequences. Similar to how "repr()" on
   bytes produces escaped byte values.

   _input_ must be a "bytes" object.

   Returns a tuple "(output, length)" where _output_ is a "bytes"
   object and _length_ is the number of bytes consumed.

codecs.escape_decode(input, errors=None)

   Decode _input_ from escape sequences back to the original bytes.

   _input_ must be a _bytes-like object_.
...
encodings.search_function(encoding)
   ...
   This function first normalizes the _encoding_ using
...
encodings.win32_code_page_search_function(encoding)
   ...
   Search for a Windows code page encoding _encoding_ of the form
   "cpXXXX".
