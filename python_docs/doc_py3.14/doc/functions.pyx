...
bin(integer, /)
   ...
   The result is a valid Python expression. If _integer_ is not a
...
<
   ...
   methods can no longer wrap other _descriptors_ such as
   "property()".
...
<
   ...
   _real_ or _imag_ argument is now deprecated; it should only be
...
eval(source, /, globals=None, locals=None)
   ...
   _globals_ and _locals_ mappings as global and local namespace.  If
   the _globals_ dictionary is present and does not contain a value
   ...
   _locals_ mapping is omitted it defaults to the _globals_
   ...
   "eval()" is called.  Note, _eval()_ will only have access to the
   _nested scopes_ (non-locals) in the enclosing environment if they
   ...
   Changed in version 3.13: The _globals_ and _locals_ arguments can
   now be passed as keywords.

   Changed in version 3.13: The semantics of the default _locals_
...
exec(source, /, globals=None, locals=None, *, closure=None)
   ...
   This function supports dynamic execution of Python code. _source_
   ...
   Note:
     ...
     When "exec" gets two separate objects as _globals_ and _locals_,
   ...
   The _closure_ argument specifies a closure–a tuple of cellvars.
   It’s only valid when the _object_ is a code object containing _free
   (closure) variables_. The length of the tuple must exactly match
   ...
   Note:
     ...
     below. Pass an explicit _locals_ dictionary if you need to see
     effects of the code on _locals_ after function "exec()" returns.
...
filter(function, iterable, /)
   ...
   _function_ is true.  _iterable_ may be either a sequence, a
...
hex(integer, /)
   ...
   prefixed with “0x”. If _integer_ is not a Python "int" object, it
...
iter(callable, sentinel, /)
   ...
   object which supports the _iterable_ protocol (the "__iter__()"
   ...
   raised. If the second argument, _sentinel_, is given, then the
   first argument must be a callable object.  The iterator created in
   this case will call _callable_ with no arguments for each call to
   its "__next__()" method; if the value returned is equal to
   _sentinel_, "StopIteration" will be raised, otherwise the value
   will be returned.
...
locals()
   ...
   of execution will return the _same_ mapping object. Changes made
   ...
   In an _optimized scope_ (including functions, generators, and
   ...
   binding changes made via the returned dict are _not_ written back
   ...
   nonlocal cell references does _not_ affect the contents of
   ...
   defined. The behavior in _optimized scopes_ is now as described
...
map(function, iterable, /, *iterables, strict=False)
   ...
   _iterable_, yielding the results.  If additional _iterables_
   ...
   is exhausted.  If _strict_ is "True" and one of the iterables is
...
open(file, mode='r', buffering=-1, encoding=None, errors=None, newline=None, closefd=True, opener=None)
   ...
   raw bytes use binary mode and leave _encoding_ unspecified.)  The
   available modes are:
   ...
   flag for "io.TextIOWrapper.reconfigure()". When no _buffering_
   ...
   * “Interactive” text files (files for which "isatty()" returns
   ...
   but any _text encoding_ supported by Python can be used. See the
   ...
   * "'xmlcharrefreplace'" is only supported when writing to a file.
     ...
     appropriate XML character reference "&#_nnn_;".
...
reversed(object, /)
   ...
   Return a reverse _iterator_.  The argument must be an object which
...
setattr(object, name, value, /)
   ...
   _name_ need not be a Python identifier as defined in Names
...
class slice(start, stop, step=None, /)
   ...
   Changed in version 3.12: Slice objects are now _hashable_ (provided
...
class super(type, object_or_type=None, /)
   ...
   The _object_or_type_ determines the _method resolution order_ to be
   searched.  The search starts from the class right after the _type_.

   For example, if "__mro__" of _object_or_type_ is "D -> B -> C -> A
   ...
   _type_ will be the enclosing class, and _obj_ will be the first
