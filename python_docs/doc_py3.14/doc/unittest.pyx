...
Changed in version 3.11: Test discovery dropped the _namespace
packages_ support. It has been broken since Python 3.7. Start
...
Changed in version 3.14: Test discovery supports _namespace package_
...
<
   ...
   assertNotIsSubclass(cls, superclass, msg=None)
      ...
      Test that _cls_ is (or is not) a subclass of _superclass_ (which
...
<
   ...
   assertNotStartsWith(s, prefix, msg=None)
      ...
      Test that the Unicode or byte string _s_ starts (or does not
      start) with a _prefix_. _prefix_ can also be a tuple of strings
      to try.
   ...
   assertNotEndsWith(s, suffix, msg=None)
      ...
      Test that the Unicode or byte string _s_ ends (or does not end)
      with a _suffix_. _suffix_ can also be a tuple of strings to try.
...
class unittest.IsolatedAsyncioTestCase(methodName='runTest')
   ...
   loop_factory
      ...
      The _loop_factory_ passed to "asyncio.Runner". Override in
...
class unittest.TestLoader
   ...
   discover(start_dir, pattern='test*.py', top_level_dir=None)
      ...
      are recorded as skips, not errors._start_dir_ can be a
      _namespace packages_.Paths are sorted before being imported so
      ...
      Changed in version 3.13: _top_level_dir_ is only stored for the
      duration of _discover_ call.

      Changed in version 3.14: _start_dir_ can once again be a
      _namespace package_.
