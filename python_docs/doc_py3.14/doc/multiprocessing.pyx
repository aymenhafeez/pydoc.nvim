...
trying to locate the _f_ function in the "__main__" module.
...
start a process.  These _start methods_ are
   ...
   _fork_
      ...
      method on any platform. Code that requires _fork_ must
...
was changed from _fork_ to _forkserver_ to retain the performance but
...
class multiprocessing.Process(group=None, target=None, name=None, args=(), kwargs={}, *, daemon=None)
   ...
   Note:
     ...
     locally defined _target_ function.Passing a callable object
     ...
     _target_ must have been defined within an importable module in
...
<
   ...
   not _spawn_. In addition, if the module is being run normally by
...
multiprocessing.get_context(method=None)
   ...
   If _method_ is "None" then the default context is returned. Note
   that if the global start method has not been set, this will set it
   to the default method. Otherwise _method_ should be "'fork'",
   "'spawn'", "'forkserver'".  "ValueError" is raised if the specified
   start method is not available.  See Contexts and start methods.
...
multiprocessing.get_start_method(allow_none=False)
   ...
   If the global start method has not been set and _allow_none_ is
   "False", then the start method is set to the default and the name
   is returned. If the start method has not been set and _allow_none_
...
class multiprocessing.pool.Pool([processes[, initializer[, initargs[, maxtasksperchild[, context]]]]])
   ...
   _processes_ is "None" then the number returned by
   ...
   Changed in version 3.13: _processes_ uses "os.process_cpu_count()"
