Python 3.14.2
*annotationlib.pyx*                           Last change: 2025 Dec 20
...
_annotations_ on modules, classes, and functions.
...
provides functionality for working directly with the _annotate
function_ that is used to evaluate annotations, such as
...
* _Stock semantics_ (default in Python 3.0 through 3.13; see **PEP
  3107** and **PEP 526**): Annotations are evaluated eagerly, as they
  are encountered in the source code.

* _Stringified annotations_ (used with "from __future__ import
  annotations" in Python 3.7 and newer; see **PEP 563**): Annotations
  are stored as strings only.

* _Deferred evaluation_ (default in Python 3.14 and newer; see **PEP
...
class annotationlib.ForwardRef
   ...
   evaluate(*, owner=None, globals=None, locals=None, type_params=None, format=Format.VALUE)
      ...
      If the _format_ argument is "VALUE" (the default), this method
      ...
      names that would otherwise be undefined. If the _format_
      ...
      "list[ForwardRef('undefined')]". If the _format_ argument is
      "STRING", the method will return "__forward_arg__".

      The _owner_ parameter provides the preferred mechanism for
      ...
      The _globals_, _locals_, and _type_params_ parameters provide a
      more precise mechanism for influencing the names that are
      available when the "ForwardRef" is evaluated. _globals_ and
      _locals_ are passed to "eval()", representing the global and
      local namespaces in which the name is evaluated. The
      _type_params_ parameter is relevant for objects created using
      ...
      namespace of a generic class "C", _type_params_ should be set to
      "C.__type_params__".
      ...
      If no _owner_, _globals_, _locals_, or _type_params_ are
...
annotationlib.call_annotate_function(annotate, format, *, owner=None)
   ...
   Call the _annotate function_ _annotate_ with the given _format_, a
   ...
   _owner_ is the object that owns the annotation function, usually a
...
annotationlib.call_evaluate_function(evaluate, format, *, owner=None)
   ...
   Call the _evaluate function_ _evaluate_ with the given _format_, a
   ...
   * "typing.TypeVarTuple.evaluate_default()", the default value of
   ...
   _owner_ is the object that owns the evaluate function, such as the
   type alias or type variable object.

   _format_ can be used to control the format in which the value is
   returned:
>
...
annotationlib.get_annotate_from_class_namespace(namespace)
   ...
   Retrieve the _annotate function_ from a class namespace dictionary
   _namespace_. Return "None" if the namespace does not contain an
...
annotationlib.get_annotations(obj, *, globals=None, locals=None, eval_str=False, format=Format.VALUE)
   ...
   _obj_ may be a callable, class, module, or other object with
   "__annotate__" or "__annotations__" attributes. Passing any other
   object raises "TypeError".

   The _format_ parameter controls the format in which annotations are
   ...
   * If _eval_str_ is true, values of type "str" will be un-stringized
     using "eval()". This is intended for use with stringized
     annotations ("from __future__ import annotations"). It is an
     error to set _eval_str_ to true with formats other than
     "Format.VALUE".

   * If _obj_ doesn’t have an annotations dict, returns an empty dict.
   ...
   * All accesses to object members and dict values are done using
   ...
   _eval_str_ controls whether or not values of type "str" are
   ...
   * If _eval_str_ is false (the default), values of type "str" are
     unchanged.

   _globals_ and _locals_ are passed in to "eval()"; see the
   documentation for "eval()" for more information. If _globals_ or
   _locals_ is "None", this function may replace that value with a
   context-specific default, contingent on "type(obj)":

   * If _obj_ is a module, _globals_ defaults to "obj.__dict__".

   * If _obj_ is a class, _globals_ defaults to
     "sys.modules[obj.__module__].__dict__" and _locals_ defaults to
     the _obj_ class namespace.

   * If _obj_ is a callable, _globals_ defaults to "obj.__globals__",
     although if _obj_ is a wrapped function (using
...
example, "get_annotations()" may call an arbitrary _annotate
function_, and "ForwardRef.evaluate()" may call "eval()" on an
