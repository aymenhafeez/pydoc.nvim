...
sys._clear_internal_caches()
   ...
   _only_ to release unnecessary references and memory blocks when
   hunting for leaks.
...
sys.getrefcount(object)
   ...
   objects are _immortal_ and have a very high refcount that does not
   reflect the actual number of references.  Consequently, do not rely
   on the returned value to be accurate, other than a value of 0 or 1.

   **CPython implementation detail:** _Immortal_ objects with a large
...
sys.hash_info
   ...
   Changed in version 3.4: Added _algorithm_, _hash_bits_,
   _seed_bits_, and _cutoff_.
...
sys.implementation
   ...
   _supports_isolated_interpreters_ is a boolean value, whether this
...
sys.intern(string)
   ...
   Interned strings are not _immortal_; you must keep a reference to
   the return value of "intern()" around to benefit from it.

sys._is_gil_enabled()

   Return "True" if the _GIL_ is enabled and "False" if it is
   disabled.
...
sys.is_finalizing()
   ...
   Return "True" if the main Python interpreter is _shutting down_.
...
sys._jit
   ...
   **CPython implementation detail:** JIT compilation is an
   _experimental implementation detail_ of CPython. "sys._jit" is not
...
sys._is_immortal(op)
   ...
   Return "True" if the given object is _immortal_, "False" otherwise.
...
sys.platform
   ...
   _at the time when Python was built_. Unless you want to test for a
...
sys.remote_exec(pid, script)
   ...
   Executes _script_, a file containing Python code in the remote
   process with the given _pid_.
   ...
   "sys.remote_exec" is raised with the _pid_ and the path to the
