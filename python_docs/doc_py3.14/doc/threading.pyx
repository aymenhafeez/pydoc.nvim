...
bypass the _global interpreter lock_ (GIL), the threading module
...
As of Python 3.13, _free-threaded_ builds can disable the GIL,
...
<
Note that subclasses can define ___slots___, but they are not thread
...
class threading.Thread(group=None, target=None, name=None, args=(), kwargs={}, *, daemon=None, context=None)
   ...
   _context_ is the "Context" value to use when starting the thread.
...
<
   join(timeout=None)
      ...
      stages of _Python finalization_ "join()" raises a
   ...
   name
      ...
      Changes to _name_ are only reflected at the OS level when the
      currently running thread is renamed. (Setting the _name_
