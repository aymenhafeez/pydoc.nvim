...
* reads and writes "gzip", "bz2", "compression.zstd", and "lzma"
  ...
  If any of these _optional modules_ are missing from your copy of
...
tarfile.open(name=None, mode='r', fileobj=None, bufsize=10240, **kwargs)
   ...
   the keyword argument _preset_ to specify the compression level of
   the file.

   For modes "'w:zst'", "'x:zst'" and "'w|zst'", "tarfile.open()"
   accepts the keyword argument _level_ to specify the compression
   level of the file. The keyword argument _options_ may also be
   passed, providing advanced Zstandard compression parameters
   described by "CompressionParameter". The keyword argument
   _zstd_dict_ can be passed to provide a "ZstdDict", a Zstandard
   ...
   the _mode_) that works with bytes. _bufsize_ specifies the
   blocksize and defaults to "20 * 512" bytes. Use this variant in
   combination with e.g. "sys.stdin.buffer", a socket _file object_ or
   ...
   |               | reading.                                     |
   +---------------+----------------------------------------------+
   | "'r|zst'"     | Open a Zstandard compressed _stream_ for     |
   |               | reading.                                     |
   +---------------+----------------------------------------------+
   ...
   |               | writing.                                     |
   +---------------+----------------------------------------------+
   | "'w|zst'"     | Open a Zstandard compressed _stream_ for     |
   |               | writing.                                     |
   +---------------+----------------------------------------------+
   ...
   Changed in version 3.12: The _compresslevel_ keyword argument also
   works for streams.

   Changed in version 3.14: The _preset_ keyword argument also works
   for streams.
...
tarfile.is_tarfile(name)
   ...
   module can read. _name_ may be a "str", file, or file-like object.
...
class tarfile.TarFile(name=None, mode='r', fileobj=None, format=DEFAULT_FORMAT, tarinfo=TarInfo, dereference=False, ignore_zeros=False, encoding=ENCODING, errors='surrogateescape', pax_headers=None, debug=0, errorlevel=1, stream=False)
   ...
   If _stream_ is set to "True" then while reading the archive info
...
TarFile.extractall(path='.', members=None, *, numeric_owner=False, filter=None)
   ...
   The _filter_ argument specifies how "members" are modified or
   rejected before extraction. See Extraction filters for details. It
   is recommended to set this explicitly only if specific _tar_
   ...
   Warning:
     ...
     _all_ unintended or insecure behavior. Read the Extraction
   ...
   Changed in version 3.14: The _filter_ parameter now defaults to
   "'data'".
...
TarFile.extractfile(member)
   ...
   existing members, "None" is returned. If _member_ does not appear
...
TarFile.addfile(tarinfo, fileobj=None)
   ...
   Add the "TarInfo" object _tarinfo_ to the archive. If _tarinfo_
   represents a non zero-size regular file, the _fileobj_ argument
   should be a _binary file_, and "tarinfo.size" bytes are read from
   it and added to the archive.  You can create "TarInfo" objects
   directly, or by using "gettarinfo()".

   Changed in version 3.13: _fileobj_ must be given for non-zero-sized
   regular files.
...
Warning:
  ...
  None of the available filters blocks _all_ dangerous archive
...
tarfile.data_filter(member, path)
   ...
   * Set user and group info ("uid", "gid", "uname", "gname") to
   ...
   Note that this filter does not block _all_ dangerous archive
...
in the _fileobj_ parameter in "TarFile.add()":
>
   import sys
   import tarfile
