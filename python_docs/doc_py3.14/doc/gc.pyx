...
gc.collect(generation=2)
   ...
   Perform a collection.  The optional argument _generation_ may be an
...
gc.set_threshold(threshold0[, threshold1[, threshold2]])
   ...
   deallocations exceeds _threshold0_, collection starts. For each
   ...
   of object allocations has not exceeded 40 times _threshold0_, the
   collection is not run.

   The fraction of the old generation that is collected is
   **inversely** proportional to _threshold1_. The larger _threshold1_
...
gc.garbage
   ...
   Changed in version 3.2: If this list is non-empty at _interpreter
   shutdown_, a "ResourceWarning" is emitted, which is silent by
...
gc.callbacks
   ...
   _info_ is a dict providing more information for the callback.  The
   following keys are currently defined:

      “generation”: The oldest generation being collected.

      “collected”: When _phase_ is “stop”, the number of objects
      successfully collected.

      “uncollectable”: When _phase_ is “stop”, the number of objects
...
gc.DEBUG_UNCOLLECTABLE
   ...
   list at _interpreter shutdown_, if it isn’t empty.
