...
urllib.request.urlopen(url, data=None, [timeout, ]*, context=None)
   ...
   Changed in version 3.13: Remove _cafile_, _capath_ and _cadefault_
   parameters: use the _context_ parameter instead.
...
urllib.request.pathname2url(path, *, add_scheme=False)
   ...
   If _add_scheme_ is false (the default), the return value omits the
   "file:" scheme prefix. Set _add_scheme_ to true to return a
   complete URL.
...
urllib.request.url2pathname(url, *, require_scheme=False, resolve_host=False)
   ...
   If _require_scheme_ is false (the default), the given value should
   omit a "file:" scheme prefix. If _require_scheme_ is set to true,
   ...
   local hostname. Otherwise, if _resolve_host_ is set to true, the
...
<
   ...
   Changed in version 3.14: The _require_scheme_ and _resolve_host_
