Python 3.14.2
*importlib.metadata.pyx*                      Last change: 2025 Dec 20
...
"importlib.metadata" address two API surfaces, one for _consumers_ and
another for _providers_. Most users are consumers, consuming metadata
...
alongside a custom importer. Such a use case calls for a _custom
provider_.
