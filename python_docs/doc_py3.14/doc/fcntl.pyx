Python 3.14.2
*fcntl.pyx*                                   Last change: 2025 Dec 20
...
_fcntl(2)_ and _ioctl(2)_ Unix manual pages for full details.
...
fcntl.fcntl(fd, cmd, arg=0, /)
   ...
   integer value, a _bytes-like object_, or a string. The type and
   size of _arg_ must match the type and size of the argument of the
   operation as specified in the relevant C documentation.

   When _arg_ is an integer, the function returns the integer return
   ...
   Note:
     ...
     If the type or the size of _arg_ does not match the type or size
   ...
   Changed in version 3.14: Add support of arbitrary _bytes-like
   objects_, not only "bytes".
...
fcntl.ioctl(fd, request, arg=0, mutate_flag=True, /)
   ...
   of interest for use as the _request_ argument can be found in the
   "termios" module, under the same names as used in the relevant C
   header files.

   The parameter _arg_ can be an integer, a _bytes-like object_, or a
   string. The type and size of _arg_ must match the type and size of
   the argument of the operation as specified in the relevant C
   documentation.

   If _arg_ does not support the read-write buffer interface or the
   _mutate_flag_ is false, behavior is as for the "fcntl()" function.

   If _arg_ supports the read-write buffer interface (like
   "bytearray") and _mutate_flag_ is true (the default), then the
   ...
   Note:
     ...
     If the type or size of _arg_ does not match the type or size of
...
fcntl.lockf(fd, cmd, len=0, start=0, whence=0, /)
   ...
   will be raised and the exception will have an _errno_ attribute set
