...
loop.create_task(coro, *, name=None, context=None, eager_start=None, **kwargs)
   ...
   An optional keyword-only _eager_start_ argument allows specifying
   if the task should execute eagerly during the call to create_task,
   or be scheduled later. If _eager_start_ is not passed the mode set
   ...
   _name_ and _context_ (if it is None), while still passing on other
   arbitrary keyword arguments (to avoid breaking backwards
   compatibility with 3.13.3).

   Changed in version 3.14: All _kwargs_ are now passed on. The
   _eager_start_ parameter works with eager task factories.
...
loop.set_task_factory(factory)
   ...
   matching "(loop, coro, **kwargs)", where _loop_ is a reference to
   the active event loop, and _coro_ is a coroutine object.  The
   callable must pass on all _kwargs_, and return a
   "asyncio.Task"-compatible object.

   Changed in version 3.13.3: Required that all _kwargs_ are passed on
   to "asyncio.Task".

   Changed in version 3.13.4: _name_ is no longer passed to task
   factories. _context_ is no longer passed to task factories if it is
   "None".

   Changed in version 3.14: _name_ and _context_ are now
...
async loop.create_server(protocol_factory, host=None, port=None, *, family=socket.AF_UNSPEC, flags=socket.AI_PASSIVE, sock=None, backlog=100, ssl=None, reuse_address=None, reuse_port=None, keep_alive=None, ssl_handshake_timeout=None, ssl_shutdown_timeout=None, start_serving=True)
   ...
   * _keep_alive_ set to "True" keeps connections active by enabling
   ...
   * _start_serving_ set to "True" (the default) causes the created
   ...
   _start_serving_ parameters. The socket option socket.TCP_NODELAY is
...
async loop.create_unix_server(protocol_factory, path=None, *, sock=None, backlog=100, ssl=None, ssl_handshake_timeout=None, ssl_shutdown_timeout=None, start_serving=True, cleanup_socket=True)
   ...
   If _cleanup_socket_ is true then the Unix socket will automatically
...
loop.set_default_executor(executor)
   ...
   _executor_ must be an instance of "ThreadPoolExecutor", which
