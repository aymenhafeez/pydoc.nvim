...
Note that methods of asyncio queues don’t have a _timeout_ parameter;
...
class asyncio.Queue(maxsize=0)
   ...
   If _maxsize_ is less than or equal to zero, the queue size is
   infinite.  If it is an integer greater than "0", then "await put()"
   blocks when the queue reaches _maxsize_ until an item is removed by
   "get()".
   ...
   shutdown(immediate=False)
      ...
      unblocked and will raise "QueueShutDown" in the formerly blocked
      thread.

      If _immediate_ is false (the default), the queue can be wound
      ...
      If _immediate_ is true, the queue is terminated immediately. The
      ...
      Use caution when using "join()" with _immediate_ set to true.
...
exception asyncio.QueueFull
   ...
   queue that has reached its _maxsize_.
