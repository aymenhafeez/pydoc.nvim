...
From Python 3.13 onward, the _GIL_ can be disabled on _free threaded_
...
prototype(vtbl_index, name[, paramflags[, iid]])
   ...
   If _iid_ is not specified, an "OSError" is raised if the COM method
   call fails. If _iid_ is specified, a "COMError" is raised instead.
...
ctypes.CopyComPointer(src, dst)
   ...
   Copies a COM pointer from _src_ to _dst_ and returns the Windows
   specific "HRESULT" value.

   If _src_ is not "NULL", its "AddRef" method is called, incrementing
   the reference count.

   In contrast, the reference count of _dst_ will not be decremented
   before assigning the new value. Unless _dst_ is "NULL", the caller
...
ctypes.create_string_buffer(size)
   ...
   If _size_ is given (and not "None"), it must be an "int". It
   specifies the size of the returned array.

   If the _init_ argument is given, it must be "bytes". It is used to
   initialize the array items. Bytes not initialized this way are set
   to zero (NUL).

   If _size_ is not given (or if it is "None"), the buffer is made one
   element larger than _init_, effectively adding a NUL terminator.

   If both arguments are given, _size_ must not be less than
   "len(init)".

   Warning:

     If _size_ is equal to "len(init)", a NUL terminator is not added.
...
ctypes.create_unicode_buffer(size)
   ...
   except _init_ must be a string and _size_ counts "c_wchar".
...
ctypes.POINTER(type, /)
   ...
   cheap. _type_ must be a ctypes type.

   **CPython implementation detail:** The resulting pointer type is
   cached in the "__pointer_type__" attribute of _type_. It is
...
ctypes.WinError(code=None, descr=None)
   ...
   "GetLastError()" is called to determine the error code. If _descr_
...
ctypes.memoryview_at(ptr, size, readonly=False)
   ...
   Return a "memoryview" object of length _size_ that references
   memory starting at _void *ptr_.

   If _readonly_ is true, the returned "memoryview" object can not be
   ...
   * size).from_address(ptr))". (While "from_address()" only takes
   integers, _ptr_ can also be given as a "ctypes.POINTER" or a
   "byref()" object.)
...
class ctypes.py_object
   ...
   Changed in version 3.14: "py_object" is now a _generic type_.
...
<
   ...
   _pack_
      ...
      allow larger _n_ than what the compiler accepts.
   ...
   _align_
      ...
      "_align_" can only _increase_ a structure’s alignment
...
<
   ...
   byte_offset
      ...
      _storage unit_; see "bit_offset".

   byte_size

      Size of the field, in bytes.

      For bitfields, this is the size of the underlying _storage
      unit_. Typically, it has the same size as the bitfield’s type.
   ...
   bit_size
      ...
      The location of a bitfield within its _storage unit_, that is,
...
ctypes.ARRAY(type, length)
   ...
   Create an array. Equivalent to "type * length", where _type_ is a
   "ctypes" data type and _length_ an integer.

   This function is _soft deprecated_ in favor of multiplication.
...
exception ctypes.COMError(hresult, text, details)
   ...
   details
      ...
      _descr_ is the textual description.  _source_ is the language-
      dependent "ProgID" for the class or application that raised the
      error.  _helpfile_ is the path of the help file.  _helpcontext_
      is the help context identifier.  _progid_ is the "ProgID" of the
