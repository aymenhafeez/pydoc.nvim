...
ssl.create_default_context(purpose=Purpose.SERVER_AUTH, *, cafile=None, capath=None, cadata=None)
   ...
   "SERVER_AUTH" as _purpose_ sets "verify_mode" to "CERT_REQUIRED"
   and either loads CA certificates (when at least one of _cafile_,
   _capath_ or _cadata_ is given) or uses
...
ssl.get_server_certificate(addr, ssl_version=PROTOCOL_TLS_CLIENT, ca_certs=None[, timeout])
   ...
   connect to the server.  If _ca_certs_ is specified, it should be a
   file containing a list of root certificates, the same format as
   used for the _cafile_ parameter in
...
SSLSocket.write(data)
   ...
   Write _data_ to the SSL socket and return the number of bytes
   written. The _data_ argument must be an object supporting the
   buffer interface.
   ...
   Changed in version 3.5: The socket timeout is no longer reset each
   time bytes are received or sent. The socket timeout is now the
   maximum total duration to write _data_.
...
SSLSocket.do_handshake(block=False)
   ...
   If _block_ is true and the timeout obtained by "gettimeout()" is
...
SSLContext.set_psk_client_callback(callback)
   ...
   is a _bytes-like object_ representing the pre-shared key. Return a
...
SSLContext.set_psk_server_callback(callback, identity_hint=None)
   ...
   used to select a corresponding PSK. The return value is a _bytes-
   like object_ representing the pre-shared key. Return a zero length
