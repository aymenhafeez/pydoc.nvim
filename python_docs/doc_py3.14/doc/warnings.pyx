...
* _message_ is a string containing a regular expression that the start
  of the warning message must match, case-insensitively.  In "-W" and
  "PYTHONWARNINGS", _message_ is a literal string that the start of
  the warning message must contain (case-insensitively), ignoring any
  whitespace at the start or end of _message_.

* _category_ is a class (a subclass of "Warning") of which the warning
  category must be a subclass in order to match.

* _module_ is a string containing a regular expression that the start
  of the fully qualified module name must match, case-sensitively.  In
  "-W" and "PYTHONWARNINGS", _module_ is a literal string that the
  fully qualified module name must be equal to (case-sensitively),
  ignoring any whitespace at the start or end of _module_.
...
Developers of applications written in Python may wish to hide _all_
...
ensure that _all_ warnings are displayed by default for the code under
...
warnings.warn(message, category=None, stacklevel=1, source=None, *, skip_file_prefixes=())
   ...
   defaults to "UserWarning".  Alternatively, _message_ can be a
   "Warning" instance, in which case _category_ will be ignored and
   ...
   filter.  The _stacklevel_ argument can be used by wrapper functions
...
<
   ...
   The _skip_file_prefixes_ keyword argument can be used to indicate
   ...
   outside of a package when a constant _stacklevel_ does not fit all
...
warnings.warn_explicit(message, category, filename, lineno, module=None, registry=None, module_globals=None, source=None)
   ...
   number, and optionally other arguments. _message_ must be a string
   and _category_ a subclass of "Warning" or _message_ may be a
   "Warning" instance, in which case _category_ will be ignored.

   _module_, if supplied, should be the module name. If no module is
   passed, the filename with ".py" stripped is used.

   _registry_, if supplied, should be the "__warningregistry__"
...
warnings.showwarning(message, category, filename, lineno, file=None, line=None)
   ...
   assigning to "warnings.showwarning". _line_ is a line of source
   code to be included in the warning message; if _line_ is not
   supplied, "showwarning()" will try to read the line specified by
   _filename_ and _lineno_.
...
warnings.formatwarning(message, category, filename, lineno, line=None)
   ...
   by _filename_ and _lineno_.

warnings.filterwarnings(action, message='', category=Warning, module='', lineno=0, append=False)

   Insert an entry into the list of warnings filter specifications.
   The entry is inserted at the front by default; if _append_ is true,
   it is inserted at the end.  This checks the types of the arguments,
   compiles the _message_ and _module_ regular expressions, and
...
@warnings.deprecated(msg, *, category=DeprecationWarning, stacklevel=1)
   ...
   may be emitted at runtime when the object is used. _static type
   checkers_ will also generate a diagnostic on usage of the
   deprecated object.
...
>
      ...
      def g(x: str) -> int: ...
<
   The warning specified by _category_ will be emitted at runtime on
   ...
   _category_ is "None", no warning is emitted at runtime. The
   _stacklevel_ determines where the warning is emitted. If it is "1"
   ...
   _category_ and _stacklevel_ arguments.
...
class warnings.catch_warnings(*, record=False, module=None, action=None, category=Warning, lineno=0, append=False)
   ...
   filter and the "showwarning()" function. If the _record_ argument
   is "False" (the default) the context manager returns "None" on
   entry. If _record_ is "True", a list is returned that is
   ...
   Changed in version 3.11: Added the _action_, _category_, _lineno_,
   and _append_ parameters.
...
depending on the value of the flag.  When _record_ is true and the
...
When _record_ is true and the flag is true, the "showwarning()"
