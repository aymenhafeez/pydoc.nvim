Python 3.14.2
*tkinter.pyx*                                 Last change: 2025 Dec 20
...
widgets
   A Tkinter user interface is made up of individual _widgets_. Each
   widget is represented as a Python object, instantiated from classes
   like "ttk.Frame", "ttk.Label", and "ttk.Button".

widget hierarchy
   Widgets are arranged in a _hierarchy_. The label and button were
   contained within a frame, which in turn was contained within the
   root window. When creating each _child_ widget, its _parent_ widget
   is passed as the first argument to the widget constructor.

configuration options
   Widgets have _configuration options_, which modify their appearance
...
geometry management
   ...
   are created. A _geometry manager_ like "grid" controls where in the
   user interface they are placed.

event loop
   Tkinter reacts to user input, changes from your program, and even
   refreshes the display only when actively running an _event loop_.
...
* Widgets are referred to by a _pathname_ in Tcl (like ".frm.btn"),
...
* Operations which are implemented as separate _commands_ in Tcl (like
  "grid" or "destroy") are represented as _methods_ on Tkinter widget
...
sequence
   ...
   _bind(3tk)_ man page, and page 201 of John Ousterhout’s book, _Tcl
   and the Tk Toolkit (2nd edition)_, for details).
...
possibly with pixel zooming and/or subsampling. Add _from_coords_
parameter to "PhotoImage" methods "copy()", "zoom()" and
"subsample()". Add _zoom_ and _subsample_ parameters to "PhotoImage"
method "copy()".
