...
Most commands return a tuple: "(type, [data, ...])" where _type_ is
...
IMAP4.idle(duration=None)
   ...
   _iterator_ protocol, and sends "DONE" upon context exit.
   ...
   The _duration_ argument sets a maximum duration (in seconds) to
   ...
   connection; _duration_ must be "None" on "IMAP4_stream"
   connections.
>
...
<
   Idler.burst(interval=0.1)
      ...
      Yield a burst of responses no more than _interval_ seconds apart
      (expressed as an "int" or "float").

      This _generator_ is an alternative to iterating one response at
...
<
   Note:
     ...
     **RFC 3501** (the IMAP protocol).  However, imaplib has
     ...
     strict, imaplib still continues to allow such tags to be created
