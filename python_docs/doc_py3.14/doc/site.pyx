...
"lib/python_X.Y[t]_/site-packages" (on Unix and macOS). (The optional
suffix “t” indicates the _free threading_ build, and is appended if
...
Changed in version 3.13: On Unix, _Free threading_ Python
...
"_name_.pth" and exists in one of the four directories mentioned
...
first and then by the _locale encoding_ if it fails.
...
site.USER_SITE
   ...
   value is "~/.local/lib/python_X.Y_[t]/site-packages" for UNIX and
   non-framework macOS builds, "~/Library/Python/_X.Y_/lib/python
   /site-packages" for macOS framework builds, and
   "_%APPDATA%_\Python\Python_XY_\site-packages" on Windows.  The
...
site.USER_BASE
   ...
   "~/Library/Python/_X.Y_" for macOS framework builds, and
   "_%APPDATA%_\Python" for Windows.  This value is used to compute
