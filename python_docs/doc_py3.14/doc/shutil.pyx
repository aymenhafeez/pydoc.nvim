...
shutil.copyfileobj(fsrc, fdst[, length])
   ...
   Copy the contents of the _file-like object_ _fsrc_ to the file-like
   ...
   "copyfileobj()" will _not_ guarantee that the destination stream
...
shutil.copyfile(src, dst, *, follow_symlinks=True)
   ...
   _src_ and _dst_ are _path-like objects_ or path names given as
   strings.
...
shutil.copymode(src, dst, *, follow_symlinks=True)
   ...
   owner, and group are unaffected.  _src_ and _dst_ are _path-like
   objects_ or path names given as strings. If _follow_symlinks_ is
...
shutil.copystat(src, dst, *, follow_symlinks=True)
   ...
   owner, and group are unaffected.  _src_ and _dst_ are _path-like
   objects_ or path names given as strings.
...
shutil.rmtree(path, ignore_errors=False, onerror=None, *, onexc=None, dir_fd=None)
   ...
   specified by _onexc_ or _onerror_ or, if both are omitted,
   ...
   If _onexc_ is provided, it must be a callable that accepts three
   ...
   _function_.  The third parameter, _excinfo_, is the exception that
   was raised. Exceptions raised by _onexc_ will not be caught.

   The deprecated _onerror_ is similar to _onexc_, except that the
...
shutil.move(src, dst, copy_function=copy2)
   ...
   If _dst_ is an existing directory or a symlink to a directory, then
   _src_ is moved inside that directory. The destination path in that
   directory must not already exist.

   If _dst_ already exists but is not a directory, it may be
   overwritten depending on "os.rename()" semantics.

   If the destination is on the current filesystem, then "os.rename()"
   is used. Otherwise, _src_ is copied to the destination using
   _copy_function_ and then removed.  In case of symlinks, a new
   symlink pointing to the target of _src_ will be created as the
   destination and _src_ will be removed.

   If _copy_function_ is given, it must be a callable that takes two
   arguments, _src_ and the destination, and will be used to copy
   _src_ to the destination if "os.rename()" cannot be used.  If the
   source is a directory, "copytree()" is called, passing it the
   _copy_function_. The default _copy_function_ is "copy2()".  Using
   "copy()" as the _copy_function_ allows the move to succeed when it
...
shutil.disk_usage(path)
   ...
   Note:
     ...
     On Unix filesystems, _path_ must point to a path within a
...
shutil.chown(path, user=None, group=None, *, dir_fd=None, follow_symlinks=True)
   ...
   Changed in version 3.13: Added _dir_fd_ and _follow_symlinks_
   parameters.
...
shutil.which(cmd, mode=os.F_OK | os.X_OK, path=None)
   ...
   If _cmd_ contains a directory component, "which()" only checks the
   specified path directly and does not search the directories listed
   in _path_ or in the system’s "PATH" environment variable.
   ...
   prepended to _path_. To avoid consulting the current working
...
<
   This is also applied when _cmd_ is a path that contains a directory
   component:
>
...
<
   ...
   longer prepended to the search path if _mode_ includes "os.X_OK"
...
shutil.make_archive(base_name, format[, root_dir[, base_dir[, verbose[, dry_run[, owner[, group[, logger]]]]]]])
   ...
   _format_ is the archive format: one of “zip” (if the "zlib" module
   ...
   Note:
     ...
     with "register_archive_format()" do not support the _root_dir_
     argument.  In this case it temporarily changes the current
     working directory of the process to _root_dir_ to perform
     archiving.
...
shutil.get_archive_formats()
   ...
   * _zstdtar_: Zstandard compressed tar-file (if the
...
shutil.register_archive_format(name, function[, extra_args[, description]])
   ...
   If _function_ has the custom attribute "function.supports_root_dir"
   set to "True", the _root_dir_ argument is passed as a keyword
   argument. Otherwise the current working directory of the process is
   temporarily changed to _root_dir_ before calling _function_. In
...
shutil.unpack_archive(filename[, extract_dir[, format[, filter]]])
   ...
   The keyword-only _filter_ argument is passed to the underlying
   unpacking function. For zip files, _filter_ is not accepted. For
   ...
   Warning:
     ...
     such security issues, but will not prevent _all_ unintended
