...
select.epoll(sizehint=-1, flags=0)
   ...
   _sizehint_ informs epoll about the expected number of events to be
   ...
   _flags_ is deprecated and completely ignored.  However, when
   ...
   Deprecated since version 3.4: The _flags_ parameter.
...
select.select(rlist, wlist, xlist, timeout=None)
   ...
   * _xlist_: wait for an “exceptional condition” (see the manual page
   ...
   Among the acceptable object types in the iterables are Python _file
   objects_ (e.g. "sys.stdin", or objects returned by "open()" or
   "os.popen()"), socket objects returned by "socket.socket()".  You
   may also define a _wrapper_ class yourself, as long as it has an
...
_O_(_highest file descriptor_) and "poll()" is _O_(_number of file
descriptors_), "/dev/poll" is _O_(_active file descriptors_).
...
devpoll.register(fd[, eventmask])
   ...
   has any pending I/O events.  _fd_ can be either an integer, or an
...
devpoll.unregister(fd)
   ...
   like the "register()" method, _fd_ can be an integer or an object
...
devpoll.poll([timeout])
   ...
   wait for events before returning. If _timeout_ is omitted, -1, or
...
"select()" is _O_(_highest file descriptor_), while "poll()" is
_O_(_number of file descriptors_).
