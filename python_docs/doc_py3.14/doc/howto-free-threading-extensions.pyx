Python 3.14.2
*howto-free-threading-extensions.pyx*         Last change: 2025 Dec 20

C API Extension Support for Free Threading
******************************************

Starting with the 3.13 release, CPython has support for running with
the _global interpreter lock_ (GIL) disabled in a configuration called
_free threading_.  This document describes how to adapt C API
...
* **Borrowed References**: C API functions that return _borrowed
  references_ may not be thread-safe if the containing object is
...
Some C API functions return _borrowed references_. These APIs are not
...
replacements that return _strong references_.
...
manage thread state even when the _GIL_ is disabled.  For example, if
...
lock acquisitions, to allow other threads to run the _cyclic garbage
collector_.
...
* Remember that the critical section macros are primarily for
  protecting access to _Python objects_ that might be involved in
...
* pypa/cibuildwheel supports the free-threaded build on Python 3.13
  and 3.14. On Python 3.14, free-threaded wheels will be built by
  default. On Python 3.13, you will need to set CIBW_ENABLE to
