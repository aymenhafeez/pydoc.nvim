...
"__annotations__", not uses _for_ annotations. If you’re looking for
...
3.10, "o.__annotations__" is guaranteed to _always_ work on Python
functions, classes, and modules.  If you’re certain the object you’re
examining is one of these three _specific_ objects, you may simply use
...
"inspect.get_annotations()": you should use three-argument "getattr()"
to access the object’s "__annotations__" attribute.
...
annotations dict of a _base class._ As an example:
>
   class Base:
       a: int = 3
       b: str = 'abc'
...
values that specifically _can’t_ be evaluated.  For example:
...
is _not_ true of modules and classes, in any Python version; those
