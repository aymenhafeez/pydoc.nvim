...
Creating a new class creates a new _type_ of object, allowing new
_instances_ of that type to be made.  Each class instance can have
...
members) are _public_ (except see below Private Variables), and all
member functions are _virtual_.  As in Modula-3, there are no
...
simply create a _new_ local variable in the innermost scope, leaving
...
object_ is created.  This is basically a wrapper around the contents
...
thing as "MyClass.f" — it is a _method object_, not a function object.
...
to "MyClass.f(x)".  In general, calling a method with a list of _n_
...
possibly surprising effects with involving _mutable_ objects such as
lists and dictionaries. For example, the _tricks_ list in the
following code should not be used as a class variable because just a
single list would be shared by all _Dog_ instances:
>
   class Dog:
...
is limited support for such a mechanism, called _name mangling_.  Any
...
_Generators_ are a simple and powerful tool for creating iterators.
