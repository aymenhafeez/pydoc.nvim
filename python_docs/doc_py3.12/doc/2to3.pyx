...
(raising "PendingDeprecationWarning" on import) and fully deprecated
in Python 3.11 (raising "DeprecationWarning").  The "2to3" tool is
...
<
Some fixers are _explicit_, meaning they aren’t run by default and
...
flag.  Note that _only_ doctests will be refactored.  This also
...
3.9 switched to a PEG parser (see **PEP 617**) while lib2to3 is using
a less flexible LL(1) parser.  Python 3.10 includes new language
syntax that is not parsable by lib2to3’s LL(1) parser (see **PEP
634**). The "lib2to3" module was marked pending for deprecation in
Python 3.9 (raising "PendingDeprecationWarning" on import) and fully
deprecated in Python 3.11 (raising "DeprecationWarning"). It will be
removed from the standard library in Python 3.13. Consider third-party
alternatives such as LibCST or parso.
